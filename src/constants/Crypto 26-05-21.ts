export const ALL_CRYPTO = {
  BTC: "Bitcoin",
  ETH: "Ethereum",
  USDT: "Tether",
  BNB: "Binance Coin",
  ADA: "Cardano",
  XRP: "XRP",
  DOGE: "Dogecoin",
  USDC: "USD Coin",
  DOT: "Polkadot",
  ICP: "Internet Computer",
  UNI: "UNICORN Token",
  BCH: "Bitcoin Cash",
  LTC: "Litecoin",
  LINK: "Chainlink",
  MATIC: "Polygon",
  XLM: "Stellar",
  ETC: "Ethereum Classic",
  BUSD: "Binance USD",
  SOL: "Solana",
  VET: "VeChain",
  WBTC: "Wrapped Bitcoin",
  THETA: "THETA",
  TRX: "TRON",
  EOS: "EOS",
  FIL: "Filecoin",
  AAVE: "Aave",
  DAI: "Dai",
  XMR: "Monero",
  NEO: "Neo",
  MKR: "Maker",
  SHIB: "SHIBA INU",
  KLAY: "Klaytn",
  BSV: "Bitcoin SV",
  XTZ: "Tezos",
  CRO: "Crypto.com Coin",
  FTT: "FTX Token",
  MIOTA: "IOTA",
  ALGO: "Algorand",
  HT: "Huobi Token",
  ATOM: "Cosmos",
  CAKE: "PancakeSwap",
  RUNE: "THORChain (ERC20)",
  LUNA: "Luna Coin",
  KSM: "Kusama",
  BTT: "BitTorrent",
  AVAX: "Avalanche",
  HBAR: "Hedera Hashgraph",
  COMP: "Compound Coin",
  LEO: "UNUS SED LEO",
  BTCB: "Bitcoin BEP2",
  DASH: "Dash",
  UST: "TerraUSD",
  TEL: "Telcoin",
  DCR: "Decred",
  EGLD: "Elrond",
  ZEC: "Zcash",
  XEM: "NEM",
  YFI: "yearn.finance",
  CEL: "Celsius",
  WAVES: "Waves",
  SNX: "Synthetix",
  HOT: "Hydro Protocol",
  SUSHI: "SushiSwap",
  TFUEL: "Theta Fuel",
  CHZ: "Chiliz",
  PAX: "Paxos Standard",
  NEAR: "NEAR Protocol",
  REV: "RChain",
  MANA: "Decentraland",
  HNT: "Helium",
  ZIL: "Zilliqa",
  STX: "Stox",
  QTUM: "Qtum",
  TUSD: "TrueUSD",
  BAT: "Basic Attention Token",
  BTG: "Bitcoin Gold",
  NEXO: "Nexo",
  ZEN: "Horizen",
  GRT: "Golden Ratio Token",
  ENJ: "Enjin Coin",
  ONT: "Ontology",
  DGB: "DigiByte",
  ONE: "BigONE Token",
  NANO: "Nano",
  OMG: "OMG Network",
  HUSD: "HUSD",
  CHSB: "SwissBorg",
  SC: "Siacoin",
  BNT: "Bancor",
  OKB: "OKB",
  UMA: "UMA",
  ZRX: "0x",
  FTM: "Fitmin Finance",
  CELO: "Celo",
  MDX: "Mandala Exchange Token",
  RVN: "Ravencoin",
  ICX: "ICON",
  ANKR: "Ankr",
  BAKE: "BakeryToken",
  XDC: "XinFin Network",
  FLOW: "Flow",
  CRV: "Curve DAO Token",
  BCD: "Bitcoin Diamond",
  KCS: "KuCoin Token",
  VGX: "Voyager Token",
  LSK: "Lisk",
  IOST: "IOST",
  LPT: "Livepeer",
  QNT: "Quant",
  AR: "Arweave",
  RSR: "Reserve Rights",
  LRC: "Loopring",
  "1INCH": "1inch",
  REN: "Ren",
  WRX: "WazirX",
  RLC: "iExec RLC",
  XVG: "Verge",
  RENBTC: "renBTC",
  DENT: "Dent",
  USDN: "Neutrino USD",
  ERG: "Ergo",
  REEF: "Reef",
  CKB: "Nervos Network",
  GT: "GateToken",
  BTMX: "BTMX",
  XVS: "Venus",
  KNC: "Kyber Network Crystal Legacy",
  OGN: "Origin Protocol",
  SNT: "Status",
  VTHO: "VeThor Token",
  SKL: "SKALE Network",
  IOTX: "IoTeX",
  GLM: "Golem",
  PROM: "Prometeus",
  STORJ: "Storj",
  CFX: "Conflux Network",
  EWT: "Energy Web Token",
  GNO: "Gnosis",
  BTCST: "Bitcoin Standard Hashrate Token",
  REP: "Augur",
  ONG: "SoMee.Social",
  OCEAN: "Ocean Protocol",
  OXT: "Orchid",
  STEEM: "Steem",
  ALPHA: "Alpha Finance Lab",
  STMX: "StormX",
  KAVA: "Kava.io",
  CTSI: "Cartesi",
  INJ: "Injective Protocol",
  NKN: "NKN",
  FUN: "FUNToken",
  WAXP: "WAX",
  SRM: "Serum",
  ARDR: "Ardor",
  WOO: "Wootrade",
  BAL: "Balancer",
  STRAX: "Stratis",
  CVC: "Civic",
  SXP: "Swipe",
  KMD: "Komodo",
  NMR: "Numeraire",
  CELR: "Celer Network",
  SAND: "The Sandbox",
  FET: "Fetch.ai",
  MED: "MediBloc",
  UBT: "Unibright",
  MVL: "MVL",
  HIVE: "Hive",
  AGI: "SingularityNET",
  VLX: "Velas",
  ORBS: "Orbs",
  MCO: "MCO",
  MAID: "MaidSafeCoin",
  ABBC: "ABBC Coin",
  UQC: "Uquid Coin",
  NU: "NuCypher",
  ANT: "ANTcoin",
  AMPL: "Ampleforth",
  WIN: "WINkLink",
  BAND: "Band Protocol",
  ZB: "ZB Token",
  BORA: "BORA",
  BTS: "Bat True Share",
  MTL: "Metal",
  ZKS: "ZKSwap",
  WAN: "Wanchain",
  MONA: "Monavale",
  JST: "JUST",
  RIF: "RSK Infrastructure Framework",
  DIVI: "Divi",
  ARK: "Ark",
  XHV: "Haven Protocol",
  COTI: "COTI",
  AVA: "Travala.com",
  XOR: "Oracolxor",
  POLY: "Polymath",
  DODO: "DODO",
  QKC: "QuarkChain",
  MLN: "Enzyme",
  SWAP: "TrustSwap",
  HEX: "HEX",
  SAFEMOON: "SafeMoon",
  WBNB: "Wrapped BNB",
  CTC: "Culture Ticket Chain",
  FEI: "Fei Protocol",
  FIDA: "Bonfida",
  AMP: "Amp",
  CCXX: "Counos X",
  TTT: "Tapcoin",
  LUSD: "Liquity USD",
  DFI: "DeFiChain",
  HBTC: "Huobi BTC",
  XYM: "Symbol",
  STETH: "StakeHound Staked Ether",
  XWC: "WhiteCoin",
  ARRR: "Pirate Chain",
  NXM: "NXM",
  THR: "ThoreCoin",
  INO: "INO COIN",
  ORC: "Originate Coin",
  RPD: "Rapids",
  vBNB: "Venus BNB",
  BEST: "Bitcoin and Ethereum Standard Token",
  OMI: "ECOMI",
  BOTX: "botXcoin",
  PUNDIX: "Pundi X[new]",
  ZLW: "Zelwin",
  HEDG: "HedgeTrade",
  BCHA: "Bitcoin Cash ABC",
  MIR: "MIR COIN",
  HNC: "Hellenic Coin",
  XIN: "Infinity Economics",
  SOLO: "Sologenic",
  ALCX: "Alchemix",
  SUSD: "sUSD",
  KLV: "Klever",
  AXS: "Axie Infinity",
  KSP: "KLAYswap Protocol",
  TRIBE: "Tribe",
  PERP: "Perpetual Protocol",
  XPRT: "Persistence",
  RAY: "Raydium",
  DAWN: "Dawn Protocol",
  ORN: "Orion Protocol",
  AKT: "Akash Network",
  YOUC: "yOUcash",
  MATH: "MATH",
  TKO: "Toko Token",
  ANC: "Anoncoin",
  TITAN: "TitanSwap",
  ETN: "Electroneum",
  vXVS: "Venus XVS",
  TLM: "Alien Worlds",
  LINA: "LINA",
  vBTC: "Venus BTC",
  KARMA: "Karma DAO",
  RPL: "Rocket Pool",
  HOGE: "Hoge Finance",
  GNY: "GNY",
  RFOX: "RedFOX Labs",
  QUICK: "QuickSwap",
  AUDIO: "Audius",
  C20: "CRYPTO20",
  MARO: "Maro",
  DRS: "Doctors Coin",
  NWC: "Newscrypto",
  UTK: "Utrust",
  SUN: "SUN",
  KEEP: "Keep Network",
  ELF: "aelf",
  META: "Metadium",
  HNS: "Handshake",
  IQ: "IQ.cash",
  PAC: "PAC Protocol",
  SYS: "Syscoin",
  TWT: "Trust Wallet Token",
  POLS: "Polkastarter",
  OXY: "Oxygen",
  NOIA: "Syntropy",
  TOMO: "TomoChain",
  STRK: "Strike",
  WNXM: "Wrapped NXM",
  FORTH: "Ampleforth Governance Token",
  EUM: "Elitium",
  SAPP: "SappChain",
  BTM: "Bytom",
  PPT: "Populous",
  BADGER: "Badger DAO",
  ALICE: "MyNeighborAlice",
  ADX: "AdEx Network",
  MLK: "MiL.k",
  PHA: "Phala Network",
  PAXG: "PAX Gold",
  BRG: "Bridge Oracle",
  HXRO: "Hxro",
  FRAX: "Frax",
  DKA: "dKargo",
  ROSE: "Oasis Network",
  XNC: "XeniosCoin",
  POWR: "Power Ledger",
  DNT: "district0x",
  vETH: "Venus ETH",
  LOTTO: "Lotto",
  DAO: "DAO Maker",
  SFP: "SafePal",
  TRAC: "OriginTrail",
  PRQ: "PARSIQ",
  SCRT: "Secret",
  MASK: "NFTX Hashmasks Index",
  NYE: "NewYork Exchange",
  BURGER: "Burger Swap",
  AION: "Aion",
  LYXe: "LUKSO",
  RNDR: "Render Token",
  MWC: "MimbleWimbleCoin",
  RLY: "Rally",
  SURE: "inSure DeFi",
  BLCT: "Bloomzed Loyalty Club Ticket",
  EPS: "Epanus",
  VAI: "VAIOT",
  BNANA: "Chimpion",
  XCM: "CoinMetro Token",
  vUSDC: "Venus USDC",
  TROY: "TROY",
  ELON: "Dogelon Mars",
  MFT: "Hifi Finance",
  KAI: "KardiaChain",
  MBN: "Membrana",
  KIN: "Kin",
  CHR: "Chromia",
  GRN: "GreenPower",
  SNM: "SONM",
  BCN: "Bytecoin",
  MX: "MX Token",
  GAS: "Gas",
  ERC20: "ERC20",
  FIRO: "Firo",
  ATRI: "Atari Token",
  TRB: "Tellor",
  LOC: "LockTrip",
  GUSD: "Gemini Dollar",
  PNK: "Kleros",
  FLM: "Flamingo",
  RAMP: "RAMP",
  IRIS: "IRISnet",
  NRG: "Energi",
  EURS: "STASIS EURO",
  LOOM: "Loom Network",
  GALA: "Gala",
  DIA: "DIA",
  HTR: "Hathor",
  EDR: "Endor Protocol",
  NRV: "Nerve Finance",
  CORE: "cVault.finance",
  MXC: "MXC",
  CRE: "Cybereits",
  SNL: "Sport and Leisure",
  KDA: "Kadena",
  UOS: "Ultra",
  TT: "Thunder Token",
  REQ: "Request",
  ASTA: "ASTA",
  LIT: "Lition",
  GRS: "Groestlcoin",
  VRSC: "VerusCoin",
  EDG: "Edgeless",
  IGNIS: "Ignis",
  EXRD: "Radix",
  SUPER: "SuperCoin",
  APL: "Apollon Limassol",
  DDX: "DerivaDAO",
  TSHP: "12Ships",
  ELA: "Elastos",
  JGN: "Juggernaut",
  aEth: "ankrETH",
  LAMB: "Lambda",
  LON: "Tokenlon Network Token",
  SERO: "Super Zero Protocol",
  HEGIC: "Hegic",
  QC: "Qcash",
  YFII: "DFI.Money",
  WOZX: "EFFORCE",
  CRU: "Crust",
  DX: "DxChain Token",
  CREAM: "Cream Finance",
  PEAK: "PEAKDEFI",
  PYR: "Vulcan Forged PYR",
  WICC: "WaykiChain",
  ALBT: "AllianceBlock",
  BOND: "Bonded Finance",
  XYO: "XYO",
  DATA: "Streamr",
  LTO: "LTO Network",
  RGT: "Rari Governance Token",
  BZRX: "bZx Protocol",
  CUMMIES: "CUMROCKET CRYPTO",
  DVI: "Dvision Network",
  WHALE: "WHALE",
  SHR: "ShareToken",
  SOLVE: "SOLVE",
  ERN: "Ethernity Chain",
  LDO: "Lido DAO Token",
  AKRO: "Akropolis",
  WTC: "Waltonchain",
  RFR: "Refract",
  PRO: "Propy",
  SFI: "saffron.finance",
  BEAM: "Beam",
  QQQ: "Poseidon Network",
  FSN: "Fusion",
  CENNZ: "Centrality",
  LBC: "LBRY Credits",
  PIVX: "PIVX",
  SBD: "Steem Dollars",
  EMC2: "Einsteinium",
  BFT: "Bitget DeFi Token",
  CITY: "City Coin",
  DAG: "Constellation",
  STPT: "Standard Tokenization Protocol",
  VSP: "Vesper",
  PZM: "PRIZM",
  VRA: "Verasity",
  CTK: "CertiK",
  DAD: "DAD",
  POND: "Marlin",
  MBL: "MovieBloc",
  ZNN: "Zenon",
  API3: "API3",
  BOA: "BOSAGORA",
  ANY: "Anyswap",
  SRK: "SparkPoint",
  BASID: "Basid Coin",
  BIFI: "Bitcoin File",
  AE: "Aeternity",
  NXS: "Nexus",
  BLZ: "Bluzelle",
  NULS: "NULS",
  RBTC: "RSK Smart Bitcoin",
  vBUSD: "Venus BUSD",
  AXEL: "AXEL",
  DEGO: "Dego Finance",
  HUM: "Humanscape",
  DSLA: "DSLA Protocol",
  OM: "MANTRA DAO",
  LA: "LATOKEN",
  BDX: "Beldex",
  VISR: "Visor.Finance",
  AERGO: "Aergo",
  AUCTION: "Bounce Token",
  RVP: "Revolution Populi",
  BEL: "Bella Protocol",
  HMR: "Homeros",
  AQT: "Alpha Quark Token",
  SPI: "Shopping",
  ARPA: "ARPA Chain",
  BTU: "BITTUP",
  STAKE: "xDai",
  CET: "CoinEx Token",
  DRGN: "Dragonchain",
  SLP: "Small Love Potion",
  HARD: "HARD Protocol",
  TRU: "Truebit",
  KRT: "TerraKRW",
  BFC: "Bit Financial",
  MITH: "Mithril",
  ULT: "Shardus",
  CUSD: "Celo Dollar",
  FREE: "Anti-Lockdown",
  BELT: "Belt",
  SNTVT: "Sentivate",
  NSBT: "Neutrino Token",
  GLCH: "Glitch",
  LQTY: "Liquity",
  BAX: "BABB",
  ATT: "Attila",
  NIM: "Nimiq",
  EOSC: "EOS Force",
  SUKU: "SUKU",
  DUSK: "Dusk Network",
  RARI: "Rarible",
  DPR: "Deeper Network",
  DGD: "DigixDAO",
  PCX: "ChainX",
  VITE: "VITE",
  MOC: "Moss Coin",
  DMCH: "Darma Cash",
  LGCY: "LGCY Network",
  PIB: "PIBBLE",
  COS: "Contentos",
  MRPH: "Morpheus.Network",
  DOCK: "Dock",
  DBC: "DeepBrain Chain",
  GTN: "GlitzKoin",
  VTC: "Vertcoin",
  HAI: "Hacken Token",
  GXC: "GXChain",
  OXEN: "Oxen",
  PAID: "PAID Network",
  FARM: "Harvest Finance",
  BAR: "FC Barcelona Fan Token",
  CTXC: "Cortex",
  BURST: "Burst",
  BIP: "Minter Network",
  VSYS: "v.systems",
  MET: "Metronome",
  PPC: "PHILLIPS PAY COIN",
  FWT: "Freeway Token",
  SLT: "Smartlands Network",
  MUSD: "mStable USD",
  NXT: "Nxt",
  FRONT: "Frontier",
  SOC: "Soda Coin",
  CXO: "CargoX",
  DMT: "Dark Matter",
  HC: "HyperCash",
  DERO: "Dero",
  OBSR: "Observer",
  RAI: "Rai Reflex Index",
  KEY: "MoMo KEY",
  ALY: "ALLY",
  GHST: "Aavegotchi",
  ARMOR: "ARMOR",
  TVK: "Terra Virtua Kolect",
  RKN: "Rakon",
  UBX: "UBIX.Network",
  YLD: "Yield",
  UNFI: "Uniswap Finance",
  BSCPAD: "BSCPAD",
  NIF: "Unifty",
  PNG: "Pangolin",
  ID: "Everest",
  AMO: "AMO Coin",
  SWTH: "Switcheo",
  QTCON: "Quiztok",
  MHC: "#MetaHash",
  FIO: "FIO Protocol",
  CVT: "Civitas Protocol",
  ILV: "Illuvium",
  HPT: "Huobi Pool Token",
  XPR: "Proton",
  UPP: "Sentinel Protocol",
  GET: "Gire Token",
  CONV: "Convergence",
  BMI: "Bridge Mutual",
  DG: "DeGate",
  WING: "WingShop",
  PXL: "PIXL",
  CVP: "PowerPool",
  PHB: "Phoenix Global",
  REVV: "REVV",
  KP3R: "Keep3rV1",
  GO: "GoChain",
  KYL: "Kylin",
  HUNT: "HUNT",
  TMTG: "The Midas Touch Gold",
  NCASH: "Nucleus Vision",
  PAI: "Project Pai",
  DIP: "Deipool",
  MOD: "Modefi",
  MAPS: "MAPS",
  VERI: "Veritaseum",
  CND: "Cindicator",
  RCN: "Ripio Credit Network",
  PDEX: "Polkadex",
  MASS: "Massnet",
  MOF: "Molecular Future",
  BMX: "BitMart Token",
  WXT: "Wirex Token",
  SKY: "Skycoin",
  SWINGBY: "Swingby",
  BNK: "Bankera",
  NFTX: "NFTX",
  XDB: "DigitalBits",
  TORN: "Tornado Cash",
  TLOS: "Telos",
  IDEX: "IDEX",
  GBYTE: "Obyte",
  FINE: "Refinable",
  UPUNK: "Unicly CryptoPunks Collection",
  INSUR: "InsurAce",
  PNT: "Penta",
  mSLV: "Mirrored iShares Silver Trust",
  ARIA20: "Arianee",
  DCN: "Dentacoin",
  DEXT: "DEXTools",
  KAN: "BitKan",
  XSGD: "XSGD",
  ZCN: "0Chain",
  CAS: "Cashaa",
  vUSDT: "Venus USDT",
  ZANO: "Zano",
  NEX: "Nash",
  PERL: "Perlin",
  AST: "AntiscamToken",
  BAN: "Banano",
  QSP: "Quantstamp",
  ANW: "Anchor Neural World",
  BF: "BitForex Token",
  mTSLA: "Mirrored Tesla",
  BZ: "BitZ Token",
  ROUTE: "Router Protocol",
  CLO: "Callisto Network",
  CFi: "CyberFi Token",
  NAS: "Nebulas",
  NEST: "NEST Protocol",
  mQQQ: "Mirrored Invesco QQQ Trust",
  RAD: "Radicle",
  DEP: "Depth Token",
  FXS: "Frax Share",
  FRM: "Ferrum Network",
  IFC: "Infinitecoin",
  GRIN: "Grin",
  VID: "VideoCoin",
  ZEE: "ZeroSwap",
  USDK: "USDK",
  XSN: "Stakenet",
  TONE: "TE-FOOD",
  BHAO: "Bithao",
  mAAPL: "Mirrored Apple",
  DVPN: "Sentinel",
  OLY: "Olyseum",
  mAMZN: "Mirrored Amazon",
  CUDOS: "CUDOS",
  QASH: "QASH",
  FOR: "ForTube",
  mNFLX: "Mirrored Netflix",
  ZEON: "ZEON",
  FLUX: "Flux Protocol",
  mTWTR: "Mirrored Twitter",
  DF: "dForce",
  TCT: "TokenClub",
  mUSO: "Mirrored United States Oil Fund",
  MUSH: "Mushroom",
  FXF: "Finxflo",
  mBABA: "Mirrored Alibaba",
  BOR: "BoringDAO",
  NEBL: "Neblio",
  JULD: "JulSwap",
  mMSFT: "Mirrored Microsoft",
  BTR: "Bitether",
  FCT: "Factom",
  GTO: "Gifto",
  IHF: "Invictus Hyperion Fund",
  WEST: "Waves Enterprise",
  USDX: "dForce USDx",
  BAAS: "BaaSid",
  BASIC: "BASIC",
  WABI: "Wabi",
  KICK: "KickToken",
  RDN: "Raiden Network Token",
  BOSON: "Boson Protocol",
  MDT: "Measurable Data Token",
  DAC: "Davinci Coin",
  SOUL: "ChainZ Arena",
  SUTER: "suterusu",
  VITAE: "Vitae",
  VIDT: "VIDT Datalink",
  SALT: "SaltSwap Finance",
  CORA: "Corra.Finance",
  DEXE: "DeXe",
  NMC: "Namecoin",
  STAX: "StableXSwap",
  AIOZ: "AIOZ Network",
  CARD: "Cardstack",
  NAV: "Navcoin",
  AOA: "Aurora",
  MCI: "Multi Channel Influencer Creater Cloud Funding Platform",
  WOW: "WOWswap",
  PROB: "ProBit Token",
  HEZ: "Hermez Network",
  DIGG: "DIGG",
  ARDX: "ArdCoin",
  VIDY: "VIDY",
  FST: "Futureswap",
  RING: "Darwinia Network",
  KDAG: "King DAG",
  CUT: "CUTcoin",
  LCX: "LCX",
  TON: "TON Crystal",
  PSG: "Paris Saint-Germain Fan Token",
  VAL: "Valkyrie Network",
  COCOS: "Cocos-BCX",
  DREP: "Drep [new]",
  LIKE: "LikeCoin",
  DOGET: "Doge Token",
  GVT: "Genesis Vision",
  APPC: "AppCoins",
  APY: "APY.Finance",
  ESD: "Empty Set Dollar",
  CDT: "Blox",
  MPH: "Morpher",
  PI: "Plian",
  GAL: "Galatasaray Fan Token",
  AUTO: "Cube",
  NLG: "Gulden",
  SIX: "SIX",
  HELMET: "Helmet.insure",
  DUCATO: "Ducato Protocol Token",
  ZAP: "Zap",
  GAME: "GameCredits",
  CRPT: "Crypterium",
  UFT: "UniLend",
  mIAU: "Mirrored iShares Gold Trust",
  PROPS: "Props Token",
  EAURIC: "Eauric",
  ALEPH: "Aleph.im",
  mVIXY: "Mirrored ProShares VIX",
  DDIM: "DuckDaoDime",
  EL: "ELYSIA",
  NEW: "Newton",
  BLOCK: "Blocknet",
  XRT: "Robonomics.network",
  BANK: "Bankless DAO",
  VIA: "Viacoin",
  BAO: "Bao Finance",
  LMT: "Lympo Market Token",
  PENDLE: "Pendle",
  vSXP: "Venus SXP",
  EVZ: "Electric Vehicle Zone",
  AWC: "Atomic Wallet Coin",
  LGO: "LGO Token",
  MTA: "mStable Governance Token: Meta (MTA)",
  MIX: "MixMarvel",
  JUL: "JustLiquidity",
  GOC: "GoCrypto Token",
  MDA: "Moeda Loyalty Points",
  RFUEL: "RioDeFi",
  SLICE: "Tranche Finance",
  DNA: "EncrypGen",
  VEE: "BLOCKv",
  OXB: "Oxbull.tech",
  DHT: "dHedge DAO",
  TRTL: "TurtleCoin",
  BIKI: "BIKI",
  vLINK: "Venus LINK",
  DAPP: "DapperCoin",
  WPR: "WePower",
  DOV: "Dovu",
  SOCKS: "Unisocks",
  WOM: "WOM Protocol",
  XCUR: "Curate",
  APIX: "APIX",
  TRY: "Try.Finance",
  BLANK: "Blank Wallet",
  RBC: "Rubic",
  TEMCO: "TEMCO",
  QRL: "Quantum Resistant Ledger",
  UNCX: "UniCrypt",
  XED: "Exeedme",
  DYN: "Dynamic",
  LCC: "Litecoin Cash",
  ANJ: "Aragon Court",
  COVER: "COVER Protocol",
  BRD: "Bread",
  MARK: "Benchmark Protocol",
  FLETA: "FLETA",
  BUX: "Buxcoin",
  STRONG: "Strong",
  MEME: "Memetic / PepeCoin",
  GMB: "GMB",
  UBXT: "UpBots",
  K21: "K21",
  MITX: "Morpheus Labs",
  ICHI: "ICHI",
  PRE: "Presearch",
  CBK: "Cobak Token",
  NCT: "Name Change Token",
  DORA: "Dora Factory",
  OPIUM: "Opium",
  SYLO: "Sylo",
  MRX: "Metrix Coin",
  BONDLY: "Bondly",
  FIS: "Stafi",
  QCX: "QuickX Protocol",
  RINGX: "RING X PLATFORM",
  AEON: "Aeon",
  BIOT: "BioPassport Token",
  PICKLE: "Pickle Finance",
  RAINI: "Rainicorn",
  LAYER: "UniLayer",
  USDJ: "USDJ",
  OAX: "OAX",
  MTV: "MultiVAC",
  vLTC: "Venus LTC",
  SPC: "SpaceChain",
  LYM: "Lympo",
  CREDIT: "PROXI",
  PAY: "TenX",
  XDN: "DigitalNote",
  ETP: "Metaverse ETP",
  TRIAS: "Trias Token (new)",
  LCMS: "LCMS",
  ACM: "Actinium",
  PART: "Particl",
  OST: "OST",
  TNB: "Time New Bank",
  DXD: "DXdao",
  MOBI: "Mobius",
  BUNNY: "Rocket Bunny",
  GLQ: "Graphlinq Protocol",
  IDV: "Idavoll Network",
  RSTR: "Ondori",
  CYC: "Cyclone Protocol",
  TRUE: "TrueChain",
  BTSE: "BTSE",
  ARCH: "Archer DAO Governance Token",
  CMT: "Comet",
  SPIKE: "Spiking",
  SWRV: "Swerve",
  LABS: "LABS Group",
  DOUGH: "PieDAO DOUGH v2",
  SHA: "Safe Haven",
  KTN: "Kattana",
  EMRX: "Emirex Token",
  OCTO: "OctoFi",
  POLK: "Polkamarkets",
  MWAT: "Restart Energy MWAT",
  FNX: "FinNexus",
  MARSH: "UnMarshal",
  AMLT: "AMLT",
  PLC: "PLATINCOIN",
  BHD: "BitcoinHD",
  S4F: "S4FE",
  SMART: "SmartCash",
  SFT: "Safex Token",
  UBQ: "Ubiq",
  FOAM: "FOAM",
  IDLE: "Idle",
  XSUTER: "xSuter",
  PTF: "PowerTrade Fuel",
  JUV: "Juventus Fan Token",
  IQN: "IQeon",
  LTX: "Lattice Token",
  EVX: "Everex",
  ELAMA: "Elamachain",
  SCC: "SCC DIGforIT",
  CS: "Credits",
  OTO: "OTOCASH",
  RIO: "Realio Network",
  ROOBEE: "ROOBEE",
  TOP: "TokenSwap",
  CWS: "Crowns",
  VNT: "VNT Chain",
  AGA: "AGA Token",
  TNC: "Trinity Network Credit",
  EASY: "EasyFi",
  PCL: "Peculium",
  ZYN: "Zynecoin",
  ISR: "Insureum",
  PLTC: "PlatonCoin",
  WGR: "Wagerr",
  BHP: "BHPCoin",
  ORAI: "Oraichain Token",
  ADK: "Aidos Kuneen",
  SAKE: "SakeToken",
  SPARTA: "Spartan Protocol",
  BTRS: "Bitball Treasure",
  BSCS: "BSC Station",
  MOON: "MoonSwap",
  TIME: "Chrono.tech",
  ASK: "Permission Coin",
  ABT: "Arcblock",
  UIP: "UnlimitedIP",
  SENSO: "SENSO",
  HPB: "High Performance Blockchain",
  TENA: "Tena [new]",
  HORD: "Hord",
  BTCZ: "BitcoinZ",
  CBC: "CryptoBossCoin",
  UDOO: "Howdoo",
  DUCK: "Unit Protocol Duck",
  YUSRA: "YUSRA",
  FKX: "FortKnoxster",
  ABL: "Airbloc",
  RAZOR: "Razor Network",
  CSP: "Caspian",
  WPP: "WPP TOKEN",
  POA: "POA",
  PLR: "Pillar",
  NVT: "NerveNetwork",
  NIOX: "Autonio",
  ODDZ: "Oddz",
  YAXIS: "yAxis",
  HAKKA: "Hakka.Finance",
  SHROOM: "Niftyx Protocol",
  ACT: "Achain",
  BEPRO: "BEPRO Network",
  SFUND: "Seedify.fund",
  DVP: "Decentralized Vulnerability Platform",
  IPX: "Tachyon Protocol",
  ABYSS: "Abyss",
  LOWB: "Loser Coin",
  DEC: "Distributed Energy Coin",
  SAN: "Santiment Network Token",
  "\u03a4BTC": "\u03c4Bitcoin",
  PMON: "PocMon",
  NAOS: "NAOS Finance",
  ENQ: "Enecuum",
  EFX: "Effect.AI",
  XIO: "Blockzero Labs",
  TRADE: "Unitrade",
  BTC2: "Bitcoin 2",
  ANCT: "Anchor",
  NPX: "NaPoleonX",
  MTH: "Monetha",
  CON: "Converter.Finance",
  AXPR: "AXPR",
  SAITO: "Saito",
  FLG: "Folgory Coin",
  HGET: "Hedget",
  DTA: "DATA",
  XMC: "Monero Classic",
  MXX: "Multiplier",
  IGG: "IG Gold",
  BDP: "BidiPass",
  ONION: "DeepOnion",
  KRL: "Kryll",
  TRV: "TrustVerse",
  TEN: "Tenet",
  INF: "Infinitus Token",
  NEWS: "PUBLISH",
  DLT: "Agrello",
  VIB: "Viberate",
  NFY: "Non-Fungible Yearn",
  MAP: "MAP Protocol",
  PPAY: "Plasma Finance",
  ARES: "Ares Protocol",
  L2: "Leverj Gluon",
  EUNO: "EUNO",
  DOS: "DEMOS",
  EXNT: "ExNetwork Token",
  HVN: "Hiveterminal Token",
  DERI: "Deri Protocol",
  BLINK: "Blockmason Link",
  FO: "FIBOS",
  EGT: "Elastic Governance",
  BUY: "Burency",
  COVAL: "Circuits of Value",
  DMD: "DMD",
  ZT: "ZBG Token",
  OCE: "OceanEx Token",
  EGG: "NestEGG Coin",
  AME: "AMEPAY",
  PUSH: "Ethereum Push Notification Service",
  ACH: "Alchemy Pay",
  MBOX: "MOBOX",
  CVN: "CVCoin",
  HTML: "HTMLCOIN",
  HZN: "Horizon Protocol",
  OUSD: "Origin Dollar",
  KONO: "Konomi Network",
  COV: "Covesting",
  PKF: "PolkaFoundry",
  DEXA: "DEXA COIN",
  QLC: "QLC Chain",
  WORLD: "World Token",
  APM: "apM Coin",
  FNB: "FinexboxToken",
  BMXX: "Multiplier",
  XPX: "ProximaX",
  YOYOW: "YOYOW",
  NOKU: "Noku",
  RTH: "Rotharium",
  MYST: "Mysterium",
  WHITE: "Whiteheart",
  EBST: "eBoost",
  APN: "Apron Network",
  UMX: "UniMex Network",
  CGG: "Chain Guardians",
  MPL: "Maple",
  RAE: "Receive Access Ecosystem",
  LEVL: "Levolution",
  MAHA: "MahaDAO",
  XEND: "Xend Finance",
  GRID: "Grid+",
  EMC: "EduMetrix Coin",
  MDS: "Midas Dollar Share",
  TERA: "TERA",
  VNLA: "Vanilla Network",
  GS: "Genesis Shards",
  TXL: "Tixl",
  HIT: "HitChain",
  CWV: "CWV Chain",
  DVC: "DragonVein",
  XMY: "Myriad",
  APYS: "APYSwap",
  DEV: "Dev Protocol",
  OPCT: "Opacity",
  XLA: "Ripple Alpha",
  BIX: "Bibox Token",
  MAN: "Matrix AI Network",
  MUSE: "Muse",
  UNISTAKE: "Unistake",
  CRD: "CryptalDash",
  TFT: "The Famous Token",
  ETHM: "Ethereum Meta",
  BTX: "BitCore",
  PMA: "PumaPay",
  NTK: "Netkoin",
  LBTC: "Lightning Bitcoin",
  UPI: "Pawtocol",
  CHI: "Chi Gastoken",
  WATCH: "yieldwatch",
  ITC: "IoT Chain",
  KEX: "Kira Network",
  EQZ: "Equalizer",
  NIX: "NIX",
  PCM: "Precium",
  CRON: "Cryptocean",
  TAU: "Lamden",
  RUFF: "Ruff",
  VEST: "VestChain",
  SWFTC: "SwftCoin",
  GEO: "GeoCoin",
  LAND: "Landbox",
  FTC: "Feathercoin",
  OCN: "Odyssey",
  vDOT: "Venus DOT",
  YOP: "Yield Optimization Platform & Protocol",
  HYVE: "Hyve",
  MTRG: "Meter Governance",
  ROOK: "KeeperDAO",
  AAB: "AAX Token",
  OIN: "OIN Finance",
  NPXSXEM: "Pundi X NEM",
  OG: "OG Fan Token",
  WEBD: "WebDollar",
  ASR: "AS Roma Fan Token",
  XCASH: "X-CASH",
  BWF: "Beowulf",
  GRLC: "Garlicoin",
  VALOR: "Valor Token",
  PROS: "ProSwap",
  URUS: "Urus",
  ASKO: "ASKO",
  JOB: "Jobchain",
  CLBK: "Cloudbric",
  AMB: "Ambrosus",
  DGTX: "Digitex",
  KAT: "Kambria",
  BOOM: "BOOM",
  SWM: "Swarm",
  PRT: "Portion",
  YAP: "Yap Stone",
  POOLZ: "Poolz Finance",
  FIC: "Filecash",
  SNGLS: "SingularDTV",
  HY: "hybrix",
  SNB: "SynchroBitcoin",
  AITRA: "Aitra",
  PRS: "PressOne",
  TKN: "Monolith",
  OPEN: "Open Governance Token",
  MATTER: "AntiMatter",
  CTI: "ClinTex CTi",
  YFL: "YF Link",
  IXC: "Ixcoin",
  GOM2: "AnimalGo",
  ETHO: "Etho Protocol",
  PLAY: "Play Royal",
  SMT: "SmartMesh",
  FCL: "Fractal",
  DAFI: "DAFI Protocol",
  HTDF: "Orient Walt",
  BTCP: "Bitcoin Private",
  "YF-DAI": "YFDAI.FINANCE",
  TKY: "THEKEY",
  CNTR: "Centaur",
  HYC: "HYCON",
  GXT: "Global X Change Token",
  BNX: "BinaryX",
  GNX: "Genaro Network",
  AUC: "Auctus",
  STARS: "Mogul Productions",
  PLU: "Pluton",
  DRC: "DRC mobility",
  RNO: "Earneo",
  TFBX: "TrueFeedBack",
  VEX: "VELOREX",
  ZOO: "ZooKeeper",
  BIDR: "BIDR",
  CTCN: "CONTRACOIN",
  MINT: "MintCoin",
  PAR: "Parallel",
  EKT: "EDUCare",
  RNT: "OneRoot Network",
  ALPACA: "Alpaca Finance",
  WIKEN: "Project WITH",
  EM: "Empow",
  ADS: "Adshares",
  INT: "INT",
  BAC: "Basis Cash",
  RAVEN: "Raven Protocol",
  TRA: "Trabzonspor Fan Token",
  TOWER: "Tower token",
  FUSE: "Fuse Network",
  HDAO: "HyperDAO",
  CRBN: "Carbon",
  REW: "Rewardiqa",
  IOV: "IOV BlockChain",
  NFT: "APENFT",
  ROOM: "OptionRoom",
  BIRD: "Bird Finance",
  LNC: "Blocklancer",
  vDAI: "Venus DAI",
  BOLT: "BOLT",
  TOKEN: "Chainswap",
  AOG: "smARTOFGIVING",
  GEEQ: "Geeq",
  PHNX: "PhoenixDAO",
  FLO: "FlourMix",
  DYP: "DeFi Yield Protocol",
  CVR: "COVIR.IO",
  CV: "carVertical",
  TKS: "Tokes",
  FOL: "Folder Protocol",
  CHP: "CoinPoker",
  GSWAP: "Gameswap",
  IDNA: "Idena",
  VIDYA: "Vidya",
  BCDT: "Blockchain Certified Data Token",
  BALPHA: "bAlpha",
  ARGON: "Argon",
  SRN: "SIRIN LABS Token",
  AYA: "Aryacoin",
  ZASH: "ZIMBOCASH",
  BREW: "CafeSwap Token",
  ZEFU: "Zenfuse",
  "0xBTC": "0xBitcoin",
  DTEP: "DECOIN",
  TIDAL: "Tidal Finance",
  RVC: "Ravencoin Classic",
  DAPS: "DAPS Coin",
  $ANRX: "AnRKey X",
  WINGS: "Wings",
  ZEBI: "Zebi Token",
  PAN: "Pantos",
  GTC: "Gitcoin",
  KCASH: "Kcash",
  DOWS: "Shadows",
  NAVI: "Natus Vincere Fan Token",
  RAZE: "Raze Network",
  GEN: "Evolution",
  UUU: "U Network",
  NORD: "Nord Finance",
  FAT: "Fatfi Protocol",
  GHOST: "GHOSTPRISM",
  GLEEC: "Gleec",
  LUA: "Lua Token",
  WSG: "Wall Street Games",
  LML: "Lisk Machine Learning",
  VBIT: "Valobit",
  TOKO: "Tokoin",
  BTZC: "BeatzCoin",
  DMST: "DMScript",
  XST: "Xstable.Protocol",
  LBA: "Cred",
  XMX: "XMax",
  ZIG: "Zigcoin",
  DGX: "Digix Gold Token",
  ONOT: "ONOToken",
  FSW: "Falconswap",
  TIPS: "FedoraCoin",
  HORUS: "HorusPay",
  TRTT: "Trittium",
  EXM: "EXMO Coin",
  MTLX: "Mettalex",
  QUN: "QunQun",
  SYNC: "SYNC Network",
  BRY: "Berry Data",
  SIG: "xSigma",
  DTX: "Databroker",
  BLK: "BlackCoin",
  DOKI: "Doki Doki Finance",
  BPS: "BitcoinPoS",
  XPM: "Primecoin",
  EPIC: "Epic Cash",
  NBX: "Netbox Coin",
  GRC: "Gridcoin",
  IDRT: "Rupiah Token",
  PHR: "Phore",
  PAINT: "MurAll",
  LET: "LinkEye",
  GRO: "Growth DeFi",
  MFG: "Smart MFG",
  CCX: "Conceal",
  SWACE: "Swace",
  AMN: "Amon",
  XFT: "Offshift",
  SPH: "Spheroid Universe",
  STEP: "Step Finance",
  BITCNY: "bitCNY",
  FAIR: "Fairmoon",
  FNT: "Falcon Project",
  ILC: "ILCOIN",
  VDG: "VeriDocGlobal",
  ESS: "Essentia",
  UAXIE: "Unicly Mystic Axies Collection",
  UMB: "Umbrella Network",
  LXT: "Litex",
  OLT: "OneLedger",
  YOU: "YOU COIN",
  MVP: "Merculet",
  CGT: "Curio Governance",
  PLOT: "PlotX",
  GEAR: "Bitgear",
  SEELE: "Seele-N",
  NYZO: "Nyzo",
  QRK: "Quark",
  UMASK: "Unicly Hashmasks Collection",
  SAFE: "SafeCoin",
  INEX: "Inex Project",
  ICNQ: "Iconic Token",
  HAPI: "HAPI",
  CPAY: "Chainpay",
  DEX: "DEX",
  NAX: "NextDAO",
  ATP: "Alaya",
  CLOAK: "CloakCoin",
  SATT: "SaTT",
  UDO: "Unido EP",
  SNOW: "SnowSwap",
  COMBO: "Furucombo",
  CRWNY: "Crowny",
  SNC: "SunContract",
  AMON: "AmonD",
  GUM: "Gourmet Galaxy",
  TERC: "TronEuropeRewardCoin",
  GSC: "Global Social Chain",
  SMG: "Smaugs NFT",
  FIN: "DeFiner",
  EXRN: "EXRNchain",
  MTC: "Metacoin",
  MTHD: "Method Finance",
  VLD: "Vetri",
  PLF: "PlayFuel",
  MASQ: "MASQ",
  DEPAY: "DePay",
  IMT: "Imsmart",
  WILD: "Wild Ride",
  XFC: "Footballcoin",
  FUND: "Unification",
  LEDU: "Education Ecosystem",
  BID: "BLOCKIDCOIN",
  AGAR: "AGAr",
  BITG: "BitGreen",
  NDX: "Indexed Finance",
  VIBE: "VIBE",
  MTN: "Medicalchain",
  XAUR: "Xaurum",
  SEPA: "Secure Pad",
  RMT: "SureRemit",
  TRIO: "Tripio",
  POT: "PotCoin",
  CAPP: "Cappasity",
  PPP: "PayPie",
  SMARTCREDIT: "SmartCredit Token",
  CPC: "CPCoin",
  DMG: "DMM: Governance",
  GIO: "Graviocoin",
  DAT: "Datum",
  PCNT: "Playcent",
  "1WO": "1World",
  AMA: "AMATEN",
  SEEN: "SEEN",
  DIME: "Dimecoin",
  ATL: "ATLANT",
  MIST: "Alchemist DeFi Mist",
  BOX: "DefiBox",
  FOC: "TheForce Trade",
  OGO: "Origo",
  BCP: "PieDAO Balanced Crypto Pie",
  BYN: "Beyond Finance",
  INSTAR: "Insights Network",
  PAT: "Patron",
  ORAO: "ORAO Network",
  XFI: "Dfinance",
  STV: "STVKE",
  AIDUS: "AIDUS TOKEN",
  INXT: "Internxt",
  D: "Denarius",
  PINK: "Pinkcoin",
  METIS: "Metis",
  XBC: "XBN Community Token",
  FEX: "FEX Token",
  COT: "CoTrader",
  USNBT: "NuBits",
  EVED: "Evedo",
  STZEN: "StakedZEN",
  "42": "42-coin",
  UT: "Ulord",
  CHAT: "ChatCoin",
  THC: "HempCoin",
  IDEA: "Ideaology",
  SUMO: "Sumokoin",
  HYPR: "HyperBurn",
  AZUKI: "Azuki",
  AVT: "Aventus",
  OK: "OKCash",
  KANGAL: "Kangal",
  FLIXX: "Flixxo",
  GIG: "Krios",
  BLY: "Blocery",
  MEETONE: "MEET.ONE",
  KUBO: "KuboCoin",
  LINKA: "LINKA",
  EQUAD: "QuadrantProtocol",
  GDAO: "Governor DAO",
  WISH: "MyWish",
  SONO: "SONO",
  TN: "TurtleNetwork",
  TUBE: "BitTube",
  BECN: "Beacon",
  SIN: "SINOVATE",
  DRT: "DomRaider",
  STA: "STABLE ASSET",
  XRC: "xRhodium",
  ACAT: "Alphacat",
  SATA: "Signata",
  SKM: "Skrumble Network",
  HLC: "HalalChain",
  KKO: "Kineko",
  XPN: "PANTHEON X",
  XTP: "Tap",
  TAD: "Tadpole Finance",
  AC: "ACoconut",
  AID: "AidCoin",
  CHONK: "Chonk",
  XPC: "eXPerience Chain",
  PMEER: "Qitmeer",
  KTON: "Darwinia Commitment Token",
  TH: "Team Heretics Fan Token",
  LUNES: "Lunes",
  PBR: "PolkaBridge",
  HYFI: "Hyper Finance",
  DAX: "DAEX",
  CURE: "CURE Farm",
  SFD: "SAFE DEAL",
  RPZX: "Rapidz",
  N8V: "NativeCoin",
  NUTS: "Squirrel Finance",
  KIT: "DexKit",
  BRZE: "Breezecoin",
  HEAT: "HEAT",
  CHADS: "CHADS VC",
  BIS: "Bismuth",
  EOSDT: "EOSDT",
  CNN: "Content Neutrality Network",
  XLT: "Nexalt",
  PLA: "PlayDapp",
  LEAD: "Lead Wallet",
  CRW: "Crown",
  QDAO: "Q DAO Governance token v1.0",
  YEE: "YEE",
  QARK: "QANplatform",
  DITTO: "Ditto",
  BART: "BarterTrade",
  NANJ: "NANJCOIN",
  WAIF: "Waifu Token",
  DDD: "Scry.info",
  ZORA: "Zoracles",
  VAULT: "VAULT",
  ADD: "Add.xyz",
  VIN: "VINchain",
  IOC: "I/O Coin",
  SIGN: "Signature Chain",
  MNTT: "MoonTrust",
  JRT: "Jarvis Network",
  SCP: "ScPrime",
  UCA: "UCA Coin",
  DFSOCIAL: "DFSocial Gaming",
  ETNA: "ETNA Network",
  JUP: "Jupiter",
  OWC: "ODUWA",
  ADB: "adbank",
  REM: "Remme",
  CAI: "Club Atletico Independiente",
  VEIL: "Veil",
  NOW: "NOW Token",
  SOTA: "SOTA Finance",
  HNST: "Honest",
  EXY: "Experty",
  OMNI: "Omni",
  COLL: "Collateral Pay",
  NTY: "Nexty",
  ZCL: "ZClassic",
  AXIS: "Axis DeFi",
  SOAR: "SOAR.FI",
  INFI: "Insured Finance",
  UGAS: "UGAS",
  MCX: "Machi X",
  PIZZA: "PizzaSwap",
  JADE: "Jade Currency",
  TENT: "TENT",
  FYZ: "Fyooz",
  INCNT: "Incent",
  GARD: "Hashgard",
  MUE: "MonetaryUnit",
  ZER: "Zero",
  CARBON: "Carboncoin",
  CATT: "Catex Token",
  TPAY: "TokenPay",
  CUR: "Curio",
  NUG: "Nuggets",
  "2KEY": "2key.network",
  XCHF: "CryptoFranc",
  vBCH: "Venus BCH",
  ALN: "Aluna.Social",
  SHPING: "SHPING",
  PGT: "Polyient Games Governance Token",
  MGO: "MobileGo",
  KTLYO: "Katalyo",
  PASC: "Pascal",
  L3P: "Lepricon",
  ETHA: "ETHA Lend",
  BIZZ: "BIZZCOIN",
  BCA: "Bitcoiva",
  ALPHR: "Alphr finance",
  CEEK: "CEEK VR",
  COR: "Coreto",
  PGN: "Pigeoncoin",
  LDFI: "Lendefi",
  SHO: "Showcase",
  RFI: "reflect.finance",
  WCT: "Waves Community Token",
  PING: "CryptoPing",
  HYDRO: "Hydro",
  CNS: "Centric Cash",
  BHC: "BillionHappiness",
  SMTY: "Smoothy",
  MER: "Mercurial Finance",
  STAR: "FileStar",
  DJV: "DEJAVE",
  YBO: "Young Boys Fan Token",
  REPO: "REPO",
  AAC: "Acute Angle Cloud",
  YEED: "YGGDRASH",
  TX: "TransferCoin",
  MNE: "Minereum",
  DOGEFI: "DOGEFI",
  RED: "RED",
  EFL: "e-Gulden",
  SPN: "Sapien",
  PRARE: "POLKARARE",
  XSR: "Xensor",
  FERA: "Fera",
  ENG: "Enigma",
  PEOS: "pEOS",
  BBC: "BigBang Core",
  UBU: "UBU",
  FOUR: "4THPILLAR TECHNOLOGIES",
  EOSDAC: "eosDAC",
  MODIC: "Modern Investment Coin",
  LKR: "Polkalokr",
  TFL: "TrueFlip",
  NSURE: "Nsure.Network",
  YIELD: "Yield Protocol",
  BNF: "BonFi",
  WINR: "JustBet",
  VRC: "VeriCoin",
  ADC: "AudioCoin",
  PVT: "Pivot Token",
  XLAB: "XcelToken Plus",
  INFS: "Infinity Esaham",
  CORN: "Popcorn",
  ORS: "ORS Group",
  YEC: "Ycash",
  TAP: "Tapmydata",
  SNET: "Snetwork",
  BCV: "BitCapitalVendor",
  TROLL: "Trollcoin",
  RISE: "Rise",
  KCAL: "Phantasma Energy",
  BXC: "Bitcoin Classic",
  GAP: "GAPS",
  YETH: "fyeth.finance",
  DFS: "Fantasy Sports",
  IOG: "Playgroundz",
  EDN: "Eden",
  YAE: "Cryptonovae",
  UFR: "Upfiring",
  TREAT: "Treat DAO",
  LUX: "LUXCoin",
  PXC: "Pixie Coin",
  MPAY: "MenaPay",
  WNL: "WinStars.live",
  BITX: "BitScreener Token",
  MOMA: "Mochi Market",
  CAT: "Cat Token",
  "1X2": "1X2 COIN",
  NBOT: "Naka Bodhi Token",
  XHI: "HiCoin",
  CBM: "CryptoBonusMiles",
  SDT: "Terra SDT",
  SUB: "Substratum",
  SMLY: "SmileyCoin",
  CPT: "Cryptaur",
  AIT: "AICHAIN",
  ZUM: "ZUM TOKEN",
  BWT: "Bittwatt",
  BOMB: "BOMB",
  POLIS: "Polis",
  BITS: "Bit",
  HYN: "Hyperion",
  DAV: "DAV Coin",
  UGOTCHI: "Unicly Aavegotchi Astronauts Collection",
  CANN: "CannabisCoin",
  AUX: "Auxilium",
  SPAZ: "Swapcoinz",
  HTN: "Heart Number",
  HGOLD: "HollyGold",
  IXT: "IXT",
  PMGT: "Perth Mint Gold Token",
  FDZ: "Friendz",
  PST: "Primas",
  CRX: "CryptEx",
  KEYFI: "KeyFi",
  ETZ: "Ether Zero",
  LOBS: "Lobstex",
  SFX: "Safex Cash",
  NFTI: "NFT Index",
  ZDR: "Zloadr",
  EOST: "EOS TRUST",
  ROYA: "Royale Finance",
  ATN: "ATN",
  BITB: "Bean Cash",
  KGO: "KIWIGO",
  LIBERTAS: "Libertas Token",
  SNN: "SeChain",
  BC: "Bitcoin Confidential",
  TOL: "Tolar",
  URQA: "UREEQA",
  WARP: "Warp Finance",
  WLT: "Wealthlocks",
  AG8: "AtromG8",
  AIDOC: "AI Doctor",
  RYO: "Ryo Currency",
  LTHN: "Lethean",
  GCC: "GuccioneCoin",
  DZI: "DeFinition",
  UBEX: "Ubex",
  DTH: "Dether",
  RET: "RealTract",
  "DEFI++": "PieDAO DEFI++",
  PTOY: "Patientory",
  KT: "Kuai Token",
  VIG: "VIG",
  ASTRO: "AstroTools",
  AZ: "Azbit",
  GMAT: "GoWithMi",
  XIOT: "Xiotri",
  MONK: "Monkey Project",
  KRB: "Karbo",
  xBTC: "xBTC",
  CCAKE: "CarrotCake",
  ROAD: "ROAD",
  OKS: "Okschain",
  PIRATE: "PirateCash",
  PHI: "PHI Token",
  PTE: "Peet DeFi",
  EQL: "Equal",
  EXP: "Exchange Payment Coin",
  BASE: "Base Protocol",
  EXCL: "ExclusiveCoin",
  FJC: "FujiCoin",
  DONUT: "Donut",
  XBY: "XTRABYTES",
  LAUNCH: "SuperLauncher",
  VIPS: "Vipstar Coin",
  CLVA: "Clever DeFi",
  XIV: "Project Inverse",
  GOLD: "Golden Token",
  HMQ: "Humaniq",
  XAMP: "Antiample",
  ALL: "Alliance Fan Token",
  BTB: "BitBall",
  HNB: "HashNet BitEco",
  PPBLZ: "Pepemon Pepeballs",
  KUE: "Kuende",
  PNY: "Peony",
  XSM: "SpectrumCash",
  SS: "Sharder",
  DOP: "Dopple Finance",
  HOMI: "HOMIHELP",
  BMC: "Blackmoon",
  XNB: "Xeonbit",
  CAJ: "Cajutel",
  XNK: "Ink Protocol",
  BXY: "Beaxy",
  CHX: "WeOwn",
  BOOST: "Boosted Finance",
  SETH: "sETH",
  EKO: "EchoLink",
  FLASH: "Flash",
  XNV: "Nerva",
  STK: "STK Coin",
  SACT: "srnArt Gallery",
  FTX: "FintruX Network",
  NDR: "Node Runners",
  YFIII: "YFIII",
  TRST: "WeTrust",
  JCT: "Japan Content Token",
  BZE: "BZEdge",
  NKC: "Nework",
  HAVY: "Havy",
  BWX: "Blue Whale EXchange",
  MPG: "Max Property Group",
  OPT: "Opus",
  MNC: "Moneynet",
  SHARE: "Seigniorage Shares",
  HBX: "Habits",
  ADM: "ADAMANT Messenger",
  LCS: "LocalCoinSwap",
  COSM: "Cosmo Coin",
  TDX: "Tidex Token",
  SHIP: "ShipChain",
  DOGEC: "DogeCash",
  XAS: "Asch",
  BBP: "Bitbot Protocol",
  ENB: "Earnbase",
  BTO: "Bottos",
  ECO: "Ormeus Ecosystem",
  BCUG: "Blockchain Cuties Universe Governance",
  TCH: "Tchain",
  RENDOGE: "renDOGE",
  COIN: "Coinbase tokenized stock FTX",
  PRCY: "PRCY Coin",
  VNXLU: "VNX Exchange",
  EVC: "Eco Value Coin",
  GOT: "GOT Token",
  NCDT: "Nuco.cloud",
  FYD: "FYDcoin",
  CLR: "Color Platform",
  ETHYS: "Ethereum Stake",
  SPD: "SPINDLE",
  MOONS: "MoonTools",
  ELEC: "Electrify.Asia",
  BOB: "Bob's Repair",
  BPC: "BackPacker Coin",
  CNB: "Coinsbit Token",
  GRG: "RigoBlock",
  DATX: "DATx",
  COFI: "CoFiX",
  DDK: "DDKoin",
  MBC: "Marblecoin",
  LOOT: "NFTLootBox",
  INNBC: "Innovative Bioresearch Coin",
  ZP: "Zen Protocol",
  ZXC: "0xcert",
  QWC: "Qwertycoin",
  BANCA: "Banca",
  UPX: "uPlexa",
  ECOM: "Omnitude",
  OWN: "OWNDATA",
  KUV: "Kuverit",
  MALW: "MalwareChain",
  OLXA: "OLXA",
  TZC: "TrezarCoin",
  IHT: "IHT Real Estate Protocol",
  FTI: "FansTime",
  ISIKC: "Isiklar Coin",
  SAR: "Saren",
  ZPR: "ZPER",
  ZRC: "ZrCoin",
  ZNZ: "ZENZO",
  NAME: "PolkaDomain",
  FRC: "Freicoin",
  TTN: "TheToken.Network",
  ERK: "Eureka Coin",
  BZNT: "Bezant",
  KDG: "Kingdom Game 4.0",
  SHIELD: "Shield Protocol",
  TOTM: "TotemFi",
  DRG: "Dragon Coins",
  TONS: "Thisoption",
  DHV: "DeHive",
  DCNTR: "Decentrahub Coin",
  FUEL: "Jetfuel Finance",
  PHT: "Lightstreams",
  EXE: "8X8 PROTOCOL",
  GPYX: "GoldenPyrex",
  NSR: "NuShares",
  FACE: "Faceter",
  BOS: "BOSCore",
  QUIN: "QUINADS",
  DANK: "MU DANK",
  OMC: "Ormeus Cash",
  ZUT: "Zero Utility Token",
  IETH: "iETH",
  LOCK: "SherLOCK Security",
  $ROPE: "Rope",
  GFX: "GamyFi Platform",
  MSR: "Masari",
  SSP: "Smartshare",
  MCASH: "Mcashchain",
  XMON: "XMON",
  DGC: "Digitalcoin",
  NOV: "Novara Calcio Fan Token",
  SHND: "StrongHands",
  CREA: "CREA",
  DIT: "Digital Insurance Token",
  ZPT: "Zeepin",
  NEXT: "NEXT",
  TRO: "Trodl",
  MIB: "MIB Coin",
  LANA: "LanaCoin",
  COVA: "COVA",
  STBU: "Stobox Token",
  ENT: "Eternity",
  INE: "IntelliShare",
  WIZ: "CrowdWiz",
  EBC: "EBCoin",
  MTX: "Matryx",
  KIMCHI: "KIMCHI.finance",
  REAL: "REAL",
  PIPT: "Power Index Pool Token",
  WAB: "WABnetwork",
  POSS: "Posscoin",
  BSCV: "BSCView",
  TRC: "Terracoin",
  TEND: "Tendies",
  "8PAY": "8PAY",
  EBRH: "ebirah",
  CDL: "CoinDeal Token",
  RTE: "Rate3",
  QCH: "QChi",
  FTN: "Fountain",
  FMTA: "Fundamenta",
  RATING: "DPRating",
  BUZZ: "BUZZCoin",
  RAGE: "Rage Fan",
  XPD: "PetroDollar",
  ETHV: "Ethereum Vault",
  TYPH: "Typhoon Network",
  NOVO: "Novo",
  DAM: "Datamine",
  BSTY: "GlobalBoost-Y",
  TFF: "Tutti Frutti",
  SGT: "snglsDAO",
  BLTG: "Block-Logic",
  PASS: "Blockpass",
  ISLA: "Insula",
  S: "Sharpay",
  BDG: "BitDegree",
  ORB: "Orbitcoin",
  GLS: "Golos Blockchain",
  JAR: "Jarvis+",
  DCT: "DECENT",
  WOWS: "Wolves of Wall Street",
  EDC: "Earn Defi Coin",
  YSEC: "Yearn Secure",
  UDOKI: "Unicly Doki Doki Collection",
  TWIN: "Twinci",
  "1MT": "1Million Token",
  ETG: "Ethereum Gold",
  KICKS: "Sessia",
  A: "Alpha Token",
  BIR: "Birake",
  HER: "HeroNode",
  MAX: "Maxcoin",
  VOX: "Vox.Finance",
  GFUN: "GoldFund",
  CSPN: "Crypto Sports",
  NCC: "NoCapCoin",
  BTDX: "Bitcloud",
  MAS: "MidasProtocol",
  TYPE: "Typerium",
  PRIA: "PRIA",
  ABX: "Arbidex",
  CHND: "Cashhand",
  DPY: "Delphy",
  KEBAB: "Kebab Token",
  POE: "Po.et",
  PAK: "Pakcoin",
  TELOS: "Teloscoin",
  BITTO: "BITTO",
  ROT: "Rotten",
  MILK2: "Spaceswap MILK2",
  PYLNT: "Pylon Network",
  QBT: "Qbao",
  BBK: "Bitblocks",
  MRK: "MARK.SPACE",
  GBX: "Gbrick",
  DOGES: "Dogeswap",
  HX: "HyperExchange",
  SIB: "SIBCoin",
  YVS: "YVS.Finance",
  INK: "Ink",
  ADT: "adToken",
  FXT: "FXT Token",
  SPHTX: "SophiaTX",
  DBIX: "DubaiCoin",
  MMO: "MMOCoin",
  USDS: "Stably USD",
  LQD: "Liquidity Network",
  XWP: "Swap",
  RMPL: "RMPL",
  MERI: "Merebel",
  MCM: "Mochimo",
  LYNC: "LYNC Network",
  ATMOS: "Atmos",
  DACS: "DACSEE",
  WOA: "Wrapped Origin Axie",
  AXE: "Axe",
  VI: "Vid",
  NASH: "NeoWorld Cash",
  CUE: "CUE Protocol",
  INN: "Innova",
  ARNX: "Aeron",
  IND: "Indorse Token",
  CRP: "Cross Finance",
  EMD: "Emerald Crypto",
  DEAL: "iDealCash",
  XBP: "BlitzPick",
  DYT: "DoYourTip",
  XDNA: "XDNA",
  TRUST: "Trustworks",
  LND: "Lendingblock",
  CTASK: "CryptoTask",
  LMY: "LunchMoney",
  ONL: "On.Live",
  PKT: "Playkey",
  ELET: "Elementeum",
  MORPH: "MORPHOSE",
  RBLX: "Rublix",
  JET: "Jetcoin",
  FYP: "FlypMe",
  X42: "x42 Protocol",
  VIKKY: "VikkyToken",
  ORMEUS: "Ormeus Coin",
  HSC: "HashCoin",
  CRAD: "CryptoAds Marketplace",
  ESBC: "ESBC",
  CBIX: "Cubiex",
  LOG: "Woodcoin",
  GMC: "GokuMarket Credit",
  KP4R: "Keep4r",
  NOR: "Noir",
  IDT: "InvestDigital",
  FUSII: "Fusible",
  ARCO: "AquariusCoin",
  SCR: "Scorum Coins",
  VIP: "Limitless VIP",
  WG0: "Wrapped Gen-0 CryptoKitties",
  TCC: "The ChampCoin",
  SHAKE: "Spaceswap SHAKE",
  SCT: "Clash Token",
  DEB: "Debitum",
  BET: "EarnBet",
  REEC: "Renewable Electronic Energy Coin",
  ART: "Maecenas",
  HQX: "HOQU",
  AT: "ABCC Token",
  SWG: "Swirge",
  HSN: "Hyper Speed Network",
  METM: "MetaMorph",
  CHEESE: "Cheesecoin",
  WCO: "Winco",
  SCL: "Sociall",
  PRIV: "PRiVCY",
  KNDC: "KanadeCoin",
  PPY: "Peerplays",
  BRDG: "Bridge Protocol",
  ETHY: "Ethereum Yield",
  BLU: "BlueCoin",
  MBONK: "megaBONK",
  TRUMP: "TrumpCoin",
  ALLOY: "HyperAlloy",
  IBFK: "\u0130stanbul Ba\u015fak\u015fehir Fan Token",
  ZCR: "ZCore",
  START: "BSCstarter",
  PEG: "Pegazus finance",
  PBT: "Primalbase Token",
  IMG: "ImageCoin",
  STBZ: "Stabilize",
  LUN: "Lunyr",
  XLR: "Solaris",
  OCT: "Octree",
  ALMX: "Almace Shards",
  SKB: "Sakura Bloom",
  SWT: "Swarm City",
  NEVA: "NevaCoin",
  TANGO: "keyTango",
  MNTP: "GoldMint",
  NETKO: "Netko",
  TCORE: "Tornado",
  PTON: "PTON",
  EGEM: "EtherGem",
  GENE: "Parkgene",
  BNTY: "Bounty0x",
  CHART: "ChartEx",
  VSL: "vSlice",
  MEC: "Megacoin",
  STM: "Streamity",
  SVX: "Savix",
  TUP: "TenUp",
  WAND: "WandX",
  WFX: "Webflix Token",
  CNUS: "CoinUs",
  EVY: "EveryCoin",
  ZENI: "Zennies",
  CEN: "Centaure",
  CO2: "Collective",
  SET: "Save Environment Token",
  PXG: "PlayGame",
  HGT: "HyperGraph",
  BCPT: "Blockmason Credit Protocol",
  SSS: "SSS Finance",
  FLP: "FLIP",
  RITO: "Ritocoin",
  BNY: "Bancacy",
  SWING: "Swing",
  MANNA: "Manna",
  TDP: "TrueDeck",
  DOPE: "DopeCoin",
  CATE: "CateCoin",
  MINTME: "MintMe.com Coin",
  BITC: "BitCash",
  CCN: "CustomContractNetwork",
  CFL: "CryptoFlow",
  TIX: "Blocktix",
  PCH: "POPCHAIN",
  AER: "Aeryus",
  BON: "Bonpay",
  CORX: "CorionX",
  EVE: "Devery",
  TSD: "True Seigniorage Dollar",
  KOBO: "Kobocoin",
  TBX: "Tokenbox",
  ZET: "Zetacoin",
  RVT: "Rivetz",
  PTC: "Pesetacoin",
  BDK: "Bidesk",
  BLUE: "Blue Swap",
  BMH: "BlockMesh",
  SWC: "Scanetchain",
  ION: "ION",
  ALT: "Alt.Estate token",
  TRIX: "TriumphX",
  ARAW: "ARAW",
  ATCC: "ATC Coin",
  TSL: "Energo",
  MICRO: "Micromines",
  OOT: "Utrum",
  GIN: "GINcoin",
  UP: "UniFi Protocol",
  BEET: "Beetle Coin",
  ASAFE: "AllSafe",
  IOP: "Internet of People",
  ARC: "Advanced Technology Coin",
  PENG: "PENG",
  SXUT: "Spectre.ai Utility Token",
  ZSC: "Zeusshield",
  WCK: "Wrapped Basic CryptoKitties",
  UCM: "UCROWDME",
  SNRG: "Synergy",
  BQQQ: "Bitsdaq",
  "1UP": "Uptrennd",
  STQ: "Storiqa",
  BOUTS: "BoutsPro",
  FUD: "FUD.finance",
  YMPL: "YMPL",
  X8X: "X8X Token",
  yTSLA: "yTSLA Finance",
  MOTO: "Motocoin",
  MOTA: "MotaCoin",
  TOC: "TouchCon",
  VDL: "Vidulum",
  BERRY: "BerrySwap",
  SHDW: "Shadow Token",
  BEZ: "Bezop",
  XCN: "Cryptonite",
  SENC: "Sentinel Chain",
  SLR: "SolarCoin",
  GEM: "Gems",
  PWR: "PWR Coin",
  BGL: "Bitgesell",
  PEPS: "PEPS Coin",
  ECU: "Decurian",
  XGM: "Defis",
  ELY: "Elysian",
  ZPAE: "ZelaaPayAE",
  MYB: "MyBit",
  KNT: "Knekted",
  UBN: "Ubricoin",
  SINS: "SafeInsure",
  CTL: "Citadel",
  DAY: "Chronologic",
  NBC: "Niobium Coin",
  UUNICLY: "Unicly Genesis Collection",
  DEM: "Deutsche eMark",
  ZLA: "Zilla",
  RBT: "Rimbit",
  SEM: "Semux",
  POP: "POP Network Token",
  CNTM: "Connectome",
  ROCK2: "ICE ROCK MINING",
  TAG: "TagCoin",
  INSN: "InsaneCoin",
  XMV: "MoneroV",
  HUSH: "Hush",
  LYRA: "Scrypta",
  CHESS: "Chess Coin",
  VRS: "Veros",
  GRFT: "Graft",
  IONC: "IONChain",
  RULER: "Ruler Protocol",
  MOR: "Mirror Farm",
  CTT: "CITEX Token",
  KFX: "KnoxFS (new)",
  IFT: "IFToken",
  POSW: "PoSW Coin",
  PHO: "Phoswap",
  AMM: "MicroMoney",
  ETX: "EthereumX",
  REB2: "Rebased",
  GZRO: "Gravity",
  KURT: "Kurrent",
  BBOO: "Panda Yield",
  "1MIL": "1MillionNFTs",
  IPL: "VouchForMe",
  HLIX: "Helix",
  BBR: "Boolberry",
  BELA: "Bela",
  NSD: "Nasdacoin",
  DCTO: "Decentralized Crypto Token",
  BCI: "Bitcoin Interest",
  ZNY: "Bitzeny",
  "2GIVE": "2GIVE",
  XPT: "XPToken.io",
  CROAT: "CROAT",
  SYN: "SynLev",
  PIPL: "PiplCoin",
  SWIFT: "SwiftCash",
  OURO: "Ouroboros",
  XSH: "SHIELD",
  INX: "Insight Protocol",
  ADI: "Aditus",
  KSEED: "Kush Finance",
  ITL: "Italian Lira",
  PLURA: "PluraCoin",
  MES: "MesChain",
  TKP: "TOKPIE",
  AC3: "AC3",
  STS: "SBank",
  ESCE: "Escroco Emerald",
  HB: "HeartBout",
  PDATA: "PDATA",
  DYNMT: "Dynamite",
  NTRN: "Neutron",
  HAND: "ShowHand",
  ARQ: "Arqma",
  OPTI: "OptiToken",
  BBO: "Bigbom",
  MODX: "MODEL-X-coin",
  JUI: "JUIICE",
  BEER: "Beer Token",
  EVIL: "Evil Coin",
  "CBIX-P": "Cubiex Power",
  FOXT: "Fox Trading",
  ORE: "Galactrum",
  PYRO: "PYRO Network",
  VOTE: "Agora",
  BGTT: "Baguette Token",
  ACED: "AceD",
  FLX: "Reflexer Ungovernance Token",
  HWC: "HollyWoodCoin",
  YFSI: "Yfscience",
  LHT: "LHT",
  KIND: "Kindcow Finance",
  FOTA: "Fortuna",
  NFXC: "NFX Coin",
  CMCT: "Crowd Machine",
  SMC: "SmartCoin",
  CIV: "Civitas",
  CVCC: "CryptoVerificationCoin",
  BTCT: "Bitcoin True",
  DUO: "DUO Network Token",
  NYEX: "Nyerium",
  CGEN: "CommunityGeneration",
  MARTK: "Martkist",
  VGW: "VegaWallet Token",
  ETGP: "Ethereum Gold Project",
  CNRG: "CryptoEnergy",
  DXT: "Datawallet",
  N0031: "nYFI",
  COB: "Cobinhood",
  DML: "Decentralized Machine Learning",
  SCB: "Space Cow Boy",
  BBS: "BBSCoin",
  CJ: "Cryptojacks",
  BSD: "Basis Dollar",
  PYRK: "Pyrk",
  SDS: "Alchemint Standards",
  MAC: "Matrexcoin",
  XPAT: "Bitnation",
  EVT: "EveriToken",
  ZUC: "ZeuxCoin",
  YTN: "YENTEN",
  TM2: "TRAXIA",
  SKIN: "SkinCoin",
  UNIFY: "Unify",
  AIM: "ModiHost",
  MFC: "MFCoin",
  BTCL: "BTC Lite",
  FLOT: "Fire Lotto",
  PTI: "Paytomat",
  ESH: "Switch",
  ETI: "EtherInc",
  WGP: "W Green Pay",
  LDC: "Leadcoin",
  FIRE: "Fire Protocol",
  PUT: "Profile Utility Token",
  WEC: "Web Coin Pay",
  ZINC: "ZINC",
  BLOC: "Blockcloud",
  INVE: "InterValue",
  MORE: "More Coin",
  PIRL: "Pirl",
  ES: "Era Swap",
  WVG0: "Wrapped Virgin Gen-0 CryptoKitties",
  DBET: "DecentBet",
  SVR: "SovranoCoin",
  PKG: "PKG Token",
  IRD: "Iridium",
  WEB: "Webcoin",
  MCT: "MicroTuber",
  SBS: "StaysBASE",
  VTX: "Vortex Defi",
  GCN: "GCN Coin",
  STAC: "StarterCoin",
  VLS: "Veles",
  OPAL: "Opal",
  GRIMM: "Grimm",
  NRP: "Neural Protocol",
  LCP: "Litecoin Plus",
  BREE: "CBDAO",
  YOC: "Yocoin",
  IC: "Ignition",
  REX: "imbrex",
  NPC: "NPCoin",
  YFFII: "YFFII Finance",
  PUX: "PolypuX",
  WTN: "Waletoken",
  DAB: "DABANKING",
  TNS: "Transcodium",
  EVN: "Evolution Finance",
  TGAME: "Truegame",
  XBI: "Bitcoin Incognito",
  ELLA: "Ellaism",
  MEXC: "MEXC Token",
  ENTRC: "EnterCoin",
  CPR: "Cipher",
  PRX: "ProxyNode",
  CRDT: "CRDT",
  RAISE: "Raise",
  TCASH: "TCASH",
  STAK: "Jigstack",
  ATB: "ATBCoin",
  BTW: "BitWhite",
  VIDZ: "PureVidz",
  ADZ: "Adzcoin",
  INRT: "INRToken",
  CCRB: "CryptoCarbon",
  ALI: "AiLink Token",
  ELTCOIN: "ELTCOIN",
  PYN: "PAYCENT",
  PGO: "PengolinCoin",
  SHMN: "StrongHands Masternode",
  QUO: "Quoxent",
  BOLI: "Bolivarcoin",
  NMP: "Neuromorphic.io",
  MOJO: "MoonJuice",
  ENRG: "Energycoin",
  FSBT: "FSBT API Token",
  SPRKL: "Sparkle Loyalty",
  GTM: "Gentarium",
  XUEZ: "Xuez",
  BTA: "Bitcoin Asset",
  CMM: "Commercium",
  GB: "GoldBlocks",
  XPY: "PayCoin",
  YI12: "Yield Stake Finance",
  BTCN: "BitcoiNote",
  ABST: "Abitshadow Token",
  GCR: "Gold Coin Reserve",
  IMGC: "ImageCash",
  YFIVE: "YFIVE FINANCE",
  ZEUS: "ZeusNetwork",
  XSTAR: "StarDEX",
  RIGEL: "Rigel Finance",
  HSS: "Hashshare",
  BEN: "BitCoen",
  BNTX: "BioNTech tokenized stock Bittrex",
  MAY: "Theresa May Coin",
  WON: "WeBlock",
  XGS: "GenesisX",
  LST: "Libartysharetoken",
  BIT: "BitMoney",
  DONU: "Donu",
  UNIT: "UNITi Protocol",
  BTAD: "Bitcoin Adult",
  DTRC: "Datarius Credit",
  RUGZ: "pulltherug.finance",
  AIX: "Aigang",
  FBN: "Fivebalance",
  EDRC: "EDRCoin",
  RBIES: "Rubies",
  CPU: "CPUcoin",
  ERO: "Eroscoin",
  BURN: "Blockburn",
  IUT: "ITO Utility Token",
  ETHPLO: "ETHplode",
  STEEP: "SteepCoin",
  BRO: "Bitradio",
  WET: "WeShow Token",
  DATP: "Decentralized Asset Trading Platform",
  BZX: "Bitcoin Zero",
  WRC: "WhiteRockCasino",
  PRIX: "Privatix",
  ENTS: "EUNOMIA",
  GDR: "Guider",
  BCZ: "Becaz",
  NRVE: "Narrative",
  aWSB: "aWSB",
  KUN: "KUN",
  TAN: "Taklimakan Network",
  CYMT: "CyberMusic",
  ACE: "ACE",
  TIC: "Thingschain",
  NZL: "Zealium",
  WCC: "WinCash",
  ATH: "Atheios",
  CALL: "Global Crypto Alliance",
  TCAT: "The Currency Analytics",
  NAVY: "BoatPilot Token",
  DFT: "DeFiat",
  ARO: "Arionum",
  PLAT: "BitGuild PLAT",
  CTRT: "Cryptrust",
  C2: "Coin2.1",
  TEAM: "TEAM (TokenStars)",
  WAGE: "Digiwage",
  SCAP: "SafeCapital",
  AREPA: "Arepacoin",
  ARION: "Arion",
  MEDIC: "MedicCoin",
  TOTO: "Tourist Token",
  JOINT: "Joint Ventures",
  SFG: "S.Finance",
  FUZE: "FUZE Token",
  CLX: "Celeum",
  FRST: "FirstCoin",
  GXX: "GravityCoin",
  POST: "PostCoin",
  BERN: "BERNcash",
  BLUR: "Blur",
  SUR: "Suretly",
  ASA: "Asura Coin",
  BSC: "BSC FARM",
  OBEE: "Obee Network",
  DRM: "DoDreamChain",
  ZNT: "Zenswap Network Token",
  RUP: "Rupee",
  GLT: "GlobalToken",
  IMP: "Ether Kingdoms Token",
  BTRN: "Biotron",
  VOT: "VoteCoin",
  VOCO: "Provoco Token",
  KWH: "KWHCoin",
  MAR: "Mchain",
  GSR: "GeyserCoin",
  MCC: "Moving Cloud Coin",
  XTA: "Italo",
  PEX: "PosEx",
  XRA: "Xriba",
  IFX24: "IFX24",
  KWATT: "4NEW",
  XIND: "INDINODE",
  PKB: "ParkByte",
  BAZT: "Baz Token",
  NAT: "Natmin Pure Escrow",
  KLKS: "Kalkulus",
  FUZZ: "FuzzBalls",
  EXMR: "EXMR FDN",
  MFTU: "Mainstream For The Underground",
  ONX: "OnX Finance",
  ICOB: "ICOBID",
  $NOOB: "noob.finance",
  DMB: "Digital Money Bits",
  B2G: "Bitcoiin",
  NCP: "Newton Coin Project",
  ARCT: "ArbitrageCT",
  NPLC: "Plus-Coin",
  MST: "MustangCoin",
  THRT: "Thrive Token",
  EBK: "ebakus",
  XCON: "Connect Coin",
  TREX: "Trexcoin",
  SCRIV: "SCRIV NETWORK",
  GIC: "Giant",
  BLAST: "SafeBlast",
  XOV: "XOVBank",
  DSR: "Desire",
  FESS: "Fesschain",
  SFCP: "SF Capital",
  IBTC: "iBTC (Synthetix)",
  CCO: "Ccore",
  XBTC21: "Bitcoin 21",
  HERB: "Herbalist Token",
  BLC: "Blakecoin",
  NOTE: "DNotes",
  TAJ: "TajCoin",
  HQT: "HyperQuant",
  SHB: "SkyHub Coin",
  PAYX: "Paypex",
  NUKO: "Nekonium",
  IG: "IGToken",
  COAL: "Coalculus",
  PNX: "Phantomx",
  NTR: "Netrum",
  ZFL: "Zuflo Coin",
  CNT: "Cyber Network Token",
  IMPL: "Impleum",
  XCO: "X-Coin",
  YFBT: "Yearn Finance Bit",
  SPK: "SparksPay",
  MCPC: "Mobile Crypto Pay Coin",
  SPT: "Spectrum",
  STREAM: "Streamit Coin",
  SEFA: "MESEFA",
  TVNT: "TravelNote",
  APR: "APR Coin",
  BNC: "Bionic",
  CARE: "Carebit",
  KEMA: "Kemacoin",
  ANON: "ANON",
  HLX: "Helex",
  LTCR: "Litecred",
  HVCO: "High Voltage",
  UVU: "CCUniverse",
  HELP: "Helpico",
  onLEXpa: "onLEXpa",
  FREC: "Freyrchain",
  ZYD: "Zayedcoin",
  SPR: "SpreadCoin",
  ITT: "Intelligent Trading Foundation",
  MXT: "MixTrust",
  DLC: "Dollarcoin",
  SHVR: "Shivers",
  BCAC: "Business Credit Alliance Chain",
  IRA: "Diligence",
  HALV: "Halving Token",
  CF: "Californium",
  SWYFTT: "SWYFT",
  AMS: "AmsterdamCoin",
  CRM: "Cream",
  MDTK: "MDtoken",
  VLT: "Bankroll Vault",
  EXO: "Exohood",
  MLM: "MktCoin",
  ACOIN: "Alchemy",
  ULG: "Ultragate",
  FLL: "Feellike",
  PFR: "Payfair",
  LTK: "LinkToken",
  PXI: "Prime-XI",
  ARB: "ARbit",
  SND: "SnodeCoin",
  GPKR: "Gold Poker",
  DISTX: "DistX",
  DOW: "DOWCOIN",
  XPA: "XPA",
  QBC: "Quebecoin",
  CSTL: "Castle",
  QUAN: "Quantis Network",
  DTC: "Datacoin",
  GOSS: "Gossip Coin",
  FTXT: "FUTURAX",
  KKC: "Kabberry Coin",
  UCN: "UChain",
  EUC: "Eurocoin",
  PAZZI: "Paparazzi",
  VEC2: "VectorAI",
  SQR: "Squorum",
  APC: "Alpha Coin",
  CREVA: "CrevaCoin",
  PAXEX: "PAXEX",
  EGX: "EagleX",
  SONG: "SongCoin",
  MILO: "MiloCoin",
  IMS: "Independent Money System",
  CAB: "Cabbage",
  RAGNA: "Ragnarok",
  DASHG: "Dash Green",
  XAP: "Apollon",
  GALI: "Galilel",
  ETHP: "ETHPlus",
  XMCT: "XMCT",
  YLC: "YoloCash",
  ALLBI: "ALL BEST ICO",
  SOCC: "SocialCoin",
  VIVID: "Vivid Coin",
  IBANK: "iBank",
  CLC: "CaluraCoin",
  ROCO: "ROIyal Coin",
  STR: "Stater",
  ELD: "Electrum Dark",
  CNNC: "Cannation",
  SCS: "SpeedCash",
  BRIA: "BriaCoin",
  AKA: "Akroma",
  JS: "JavaScript Token",
  CXT: "Coinonat",
  PLNC: "PLNcoin",
  VOLT: "Bitvolt",
  IBS: "IBStoken",
  KTS: "Klimatas",
  NOAHP: "Noah Coin",
  MOX: "MoX",
  WBB: "Wild Beast Block",
  ICON: "Iconic",
  DIN: "Dinero",
  BUL: "Bulleon",
  SANDG: "Save and Gain",
  MOONSHOT: "Moonshot",
  MRI: "Mirai",
  MNP: "MNPCoin",
  FLC: "Flowchain",
  AAA: "Abulaba",
  PONZI: "PonziCoin",
  LTCU: "LiteCoin Ultra",
  STO: "Storeum",
  CYL: "Crystal Token",
  XPTX: "PlatinumBAR",
  NANOX: "Project-X",
  QBIC: "Qbic",
  KZC: "KZ Cash",
  ARGUS: "Argus",
  WXC: "WXCOINS",
  MPT: "MeetPle",
  HDI: "HEIDI",
  LABX: "Stakinglab",
  INNBCL: "Innovative Bioresearch Classic",
  CONX: "Concoin",
  BENZ: "Benz",
  FLT: "Flit Token",
  CVA: "Crypto Village Accelerator",
  NBXC: "Nibble",
  AGLT: "Agrolot",
  SXC: "SXC Token",
  WETH: "WETH",
  CNC: "Global China Cash",
  AKITA: "Akita Inu",
  BTCDOWN: "BTCDOWN",
  DOGGY: "DOGGY",
  OKT: "OKExChain",
  XCH: "Chia Network",
  BNBUP: "BNBUP",
  CDAI: "Compound Dai",
  CUSDC: "Compound USD Coin",
  ETHUP: "ETHUP",
  ASM: "Assemble Protocol",
  PIG: "Pig Finance",
  LF: "Linkflow Finance",
  KISHU: "Kishu Inu",
  MVC: "Maverick Chain",
  ATM: "Atletico De Madrid Fan Token",
  MEPAD: "MemePad",
  ADAUP: "ADAUP",
  BTCUP: "BTCUP",
  XT: "ExtStock Token",
  PLEX: "MinePlex",
  BNBDOWN: "BNBDOWN",
  ETH3L: "Amun Ether 3x Daily Long",
  XRPUP: "XRPUP",
  COPS: "COPS FINANCE",
  ALP: "ALP Coin",
  ADADOWN: "ADADOWN",
  XRPDOWN: "XRPDOWN",
  ETH3S: "Amun Ether 3x Daily Short",
  DOTUP: "DOTUP",
  DFYN: "Dfyn Network",
  SUSHIUP: "SUSHIUP",
  PARA: "Paralink Network",
  WHT: "Wrapped Huobi Token",
  BETH: "Beacon ETH",
  EDOGE: "ElonDoge",
  CLU: "CluCoin",
  GAT: "Global AEX Token",
  ETHBULL: "3X Long Ethereum Token",
  CSPR: "Casper",
  ETHDOWN: "ETHDOWN",
  COW: "CoinWind",
  LEASH: "Doge Killer",
  SHLD: "Shield Finance",
  AAVEDOWN: "AAVEDOWN",
  LTCUP: "LTCUP",
  BTC3L: "Amun Bitcoin 3x Daily Long",
  FEG: "FEGtoken",
  AUDT: "Australian Dollar Token",
  CARDS: "CARD.STARTER",
  HOO: "Hoo Token",
  FILDA: "Filda",
  KOK: "KOK",
  EMAX: "EthereumMax",
  VALUE: "Value Liquidity",
  XNO: "Xeno Token",
  LHB: "Lendhub",
  NAFT: "Nafter",
  ERSDL: "UnFederalReserve",
  LINKUP: "LINKUP",
  DPI: "DeFi Pulse Index",
  BAGS: "Basis Gold Share",
  NBS: "New BitShares",
  BTC3S: "Amun Bitcoin 3x Daily Short",
  YFIUP: "YFIUP",
  FootballStars: "Football Stars",
  CHT: "CoinHe Token",
  EPAN: "Paypolitan Token",
  AAVEUP: "AAVEUP",
  CUSDT: "Compound USDT",
  SXPUP: "SXPUP",
  GERA: "Gera Coin",
  KINE: "KINE",
  SBTC: "Super Bitcoin",
  UNIUP: "UNIUP",
  XSUSHI: "xSUSHI",
  ASS: "Australian Safe Shepherd",
  YFIDOWN: "YFIDOWN",
  G999: "G999",
  TRR: "Terran Coin",
  ZOOT: "Zoo Token",
  SXPDOWN: "SXPDOWN",
  DOTDOWN: "DOTDOWN",
  UNIDOWN: "UNIDOWN",
  BULL: "BuySell",
  "10SET": "Tenset",
  XRPBULL: "3x Long XRP Token",
  IDK: "IDK",
  KALLY: "Polkally",
  LAT: "PlatON",
  GOF: "Golff",
  YOOSHI: "YooShi",
  HOKK: "Hokkaidu Inu",
  VRX: "VEROX",
  YFX: "Your Future Exchange",
  HOTCROSS: "Hot Cross",
  CNNS: "CNNS",
  VELO: "Velo",
  CNZ: "Coinzo Token",
  "ETH2X-FLI": "ETH 2x Flexible Leverage Index",
  ROCKS: "Social Rocket",
  USDF: "Dollar Protocol",
  OSB: "OASISBloc",
  XLMUP: "XLMUP",
  GERO: "GeroWallet",
  MSB: "Misbloc",
  XTZUP: "XTZUP",
  DOTX: "DeFi of Thrones",
  CELL: "Cellframe",
  DFY: "Defi For You",
  WEX: "WaultSwap",
  GINU: "Green Shiba Inu",
  BEAR: "BEAR Coin",
  SANSHU: "Sanshu Inu",
  MATICBULL: "3X Long Matic Token",
  TPT: "TokenPocket",
  LINKDOWN: "LINKDOWN",
  GHX: "GamerCoin",
  EURX: "eToro Euro",
  STN: "Stone DeFi",
  BCVT: "BitcoinVend",
  MOB: "MobileCoin",
  BITCI: "Bitcicoin",
  WISE: "Wise",
  EOSDOWN: "EOSDOWN",
  ADABULL: "3X Long Cardano Token",
  EOSBULL: "3x Long EOS Token",
  CHIBI: "Chibi Inu",
  BKK: "BKEX Token",
  NOA: "NOA PLAY",
  SAFEMARS: "Safemars",
  SKEY: "SmartKey",
  LEMD: "Lemond",
  WIFI: "Wifi Coin",
  $KEI: "Keisuke Inu",
  POOL: "PoolTogether",
  VRT: "Venus Reward Token",
  FORESTPLUS: "The Forbidden Forest",
  MERL: "Merlin",
  VBNT: "Bancor Governance Token",
  FFA: "Cryptofifa",
  XFUND: "xFund",
  wCRES: "Wrapped CrescoFin",
  O3: "O3Swap",
  IBP: "Innovation Blockchain Payment",
  SSX: "SOMESING",
  GRAP: "GRAP",
  KOBE: "Shabu Shabu Finance",
  SDX: "SwapDEX",
  COPE: "Cope",
  FILUP: "FILUP",
  LIEN: "Lien",
  BRZ: "Brazilian Digital Token",
  MILF: "Moms I'd Like to Fund",
  FDO: "Firdaos",
  LTRBT: "LITTLE RABBIT",
  STND: "Standard Protocol",
  ZCX: "Unizen",
  VEN: "ImpulseVen",
  AQUAGOAT: "AquaGoat.Finance",
  STC: "Student Coin",
  WASP: "WanSwap",
  FXC: "FUTUREXCRYPTO",
  TRXUP: "TRXUP",
  NET: "NEXT",
  POLC: "Polkacity",
  AOS: "AOS",
  UDT: "Unlock Protocol",
  WWAN: "Wrapped WAN",
  ZABAKU: "ZABAKU INU",
  TKINU: "Tsuki Inu",
  WMATIC: "Wrapped Matic",
  TAI: "TAI",
  HANDY: "Handy",
  OCC: "Occam.Fi",
  COSHI: "CoShi Inu",
  ECELL: "Consensus Cell Network",
  EOSBEAR: "3x Short EOS Token",
  TRXDOWN: "TRXDOWN",
  SUSHIBULL: "3X Long Sushi Token",
  KTT: "K-Tune",
  JASMY: "Jasmy",
  ARTE: "ethArt",
  WANUSDT: "wanUSDT",
  POL: "Proof Of Liquidity",
  BSCX: "BSCEX",
  PERI: "PERI Finance",
  BIA: "Bilaxy Token",
  BNBBULL: "3X Long BNB Token",
  ADEL: "Akropolis Delphi",
  DOGEBULL: "3X Long Dogecoin Token",
  POLX: "Polylastic",
  PKOIN: "Pkoin",
  VCK: "28VCK",
  LAR: "LinkArt",
  OVR: "OVR",
  FREN: "Frenchie Network",
  DEFT: "DeFi Factory Token",
  OHM: "Olympus",
  DAVP: "Davion",
  MCB: "MCDEX",
  TARA: "Taraxa",
  TCFX: "Tcbcoin",
  FLOAT: "Float Protocol: Float",
  BEZOGE: "Bezoge Earth",
  CHIHUA: "Chihua Token",
  BEC: "Betherchip",
  MBX: "MobiePay",
  BDPI: "Interest Bearing Defi Pulse Index",
  pBTC35A: "pBTC35A",
  BCMC1: "BeforeCoinMarketCap",
  FSCC: "Fisco Coin",
  arNXM: "Armor NXM",
  FNK: "FunKeyPay",
  MCAN: "Medican Coin",
  UFC: "Union Fair Coin",
  LOL: "LOLTOKEN",
  LPOOL: "Launchpool",
  SEED: "Sesameseed",
  NFTART: "NFT Art Finance",
  DDOS: "disBalancer",
  LION: "Lion Token",
  POLA: "Polaris Share",
  VX: "ViteX Coin",
  TCP: "Token CashPay",
  WEMIX: "WEMIX",
  ATFI: "Atlantic Finance Token",
  CCT: "Coupon Chain",
  DC: "DeepCoin",
  BONFIRE: "Bonfire",
  WAR: "YieldWars",
  TENFI: "TEN",
  CFXQ: "CFX Quantum",
  CSC: "CasinoCoin",
  GTH: "Gather",
  PEARL: "Pearl",
  LOCG: "LOCGame",
  AGOV: "AGOV (ANSWER Governance)",
  pETH18C: "pETH18C",
  HOPE: "Hope",
  CVX: "Convex Finance",
  SBGO: "Bingo Share",
  LEX: "Elxis",
  MDM: "Medium",
  PET: "Battle Pets",
  wSIENNA: "Sienna (ERC20)",
  TOPC: "TopChain",
  GBPU: "Upper Pound",
  BTCV: "BitcoinV",
  YFO: "YFIONE",
  DEUS: "DEUS Finance",
  SAFEPLUTO: "SAFEPLUTO69",
  SKLAY: "sKLAY",
  N0001: "nHBTC",
  PALG: "PalGold",
  CHS: "CheeseSwap",
  ZERO: "Zero Exchange",
  CXC: "CAPITAL X CELL",
  DEA: "DEUS Finance DEA",
  XLMDOWN: "XLMDOWN",
  BPCN: "BlipCoin",
  HODL: "hodlearn",
  HYDRA: "Hydra",
  EURU: "Upper Euro",
  ETHBEAR: "3X Short Ethereum Token",
  LABRA: "LabraCoin",
  BOG: "Bogged Finance",
  TBCC: "TBCC Labs",
  EC: "Eternal Cash",
  NII: "Nahmii",
  KABOSU: "Kabosu",
  Mars: "Mars",
  SHIH: "Shih Tzu",
  EPK: "EpiK Protocol",
  SALE: "DxSale Network",
  VARC: "VARC",
  FTS: "Fortress Lending",
  LINKBULL: "3X Long Chainlink Token",
  ZDEX: "Zeedex",
  ISP: "Ispolink",
  ELT: "Elite Swap",
  CHINU: "Chubby Inu",
  VKNF: "VKENAF",
  XTZDOWN: "XTZDOWN",
  COMB: "Combine.finance",
  HDAC: "Hdac",
  WOOP: "Woonkly Power",
  GST2: "Gas Token Two",
  BDCC: "BDCC Bitica COIN",
  FIT: "FINANCIAL INVESTMENT TOKEN",
  BTCBAM: "Bitcoin Bam",
  WOMI: "Wrapped ECOMI",
  WOOFY: "Woofy",
  MARS: "Marscoin",
  SHDC: "SHD CASH",
  ICH: "iCherry Finance",
  MINA: "Mina",
  CHOW: "Chow Chow",
  DINO: "DinoExchange",
  USDU: "Upper Dollar",
  GAMESAFE: "Gamesafe.io",
  PRV: "PrivacySwap",
  JFI: "JackPool.finance",
  SNOGE: "Snoop Doge",
  eMTRG: "Meter Governance mapped by Meter.io",
  GOD: "Game Of DeFi",
  FILDOWN: "FILDOWN",
  DHC: "DeltaHub Community",
  IMX: "Impermax",
  UNIQ: "Uniqly",
  JSHIBA: "Jomon Shiba",
  SATOZ: "Satozhi",
  YAM: "YAM v1",
  QBZ: "QUEENBEE",
  MFI: "Marginswap",
  TPAD: "Trustpad",
  UNO: "Unobtanium",
  ROC: "Roxe Cash",
  BST: "BlockStamp",
  INV: "Inverse Finance",
  ELONGATE: "ElonGate",
  BQT: "Blockchain Quotations Index Token",
  LRG: "Largo Coin",
  DHX: "DataHighway",
  DDS: "DDS.Store",
  TWERK: "Twerk Finance",
  CCA: "Counos Coin",
  TBAKE: "Bakery Tools",
  QI: "QiSwap",
  YO: "Yobit Token",
  HNZO: "Hanzo Inu",
  BITSZ: "Bitsz",
  ACS: "ACryptoS",
  ARX: "ARCS",
  MOONPIRATE: "MoonPirate",
  FLY: "Franklin",
  CHAIN: "Chain Games",
  SMI: "SafeMoon Inu",
  CVNT: "Content Value Network",
  PIGX: "PIGX",
  XUC: "Exchange Union",
  OXO: "OXO.Farm",
  ADP: "Adappter Token",
  NEKO: "Neko Network",
  XPOSE: "Xpose",
  NODE: "PocketNode",
  BCHBULL: "3x Long Bitcoin Cash Token",
  DFD: "DefiDollar DAO",
  COOK: "Cook Protocol",
  HOP: "HOPPY",
  LPL: "LinkPool",
  HFI: "HecoFi",
  IBVOL: "Inverse Bitcoin Volatility Token",
  LBK: "LegalBlock",
  ZUSD: "ZUSD",
  SG: "SocialGood",
  RICH: "Rich Maker",
  FND: "Fundum Capital",
  LTCDOWN: "LTCDOWN",
  RENA: "Rena Finance",
  REVO: "Revo Network",
  MGP: "MangoChain",
  INTO: "Infiniti",
  MOMO: "Momo Protocol",
  XAUT: "Tether Gold",
  CZRX: "Compound 0x",
  USE: "Usechain Token",
  LPNT: "Luxurious Pro Network Token",
  HAPPY: "HappyCoin",
  $KING: "King Swap",
  PAYT: "PayAccept",
  ETCBULL: "3X Long Ethereum Classic Token",
  DICK: "Dick",
  BYTS: "Bytus",
  CUB: "Crypto User Base",
  BDT: "Block Duelers NFT Battles",
  MVEDA: "Medicalveda",
  FUNDX: "Funder One Capital",
  YFPI: "Yearn Finance Passive Income",
  PIT: "Pitbull",
  HOPR: "HOPR",
  DAOFI: "DAOFi",
  BLACK: "eosBLACK",
  HLP: "HLP Token",
  DIS: "TosDis",
  SUPERBID: "Superbid",
  EDDA: "EDDASwap",
  BPLC: "BlackPearl Token",
  CTRFI: "Chester.Moon",
  KPAD: "KickPad",
  XBE: "XBE Token",
  MTS: "Metis",
  BCX: "BitcoinX",
  NTVRK: "Netvrk",
  BAFE: "Bafe io",
  INDEX: "Index Cooperative",
  BSYS: "BSYS",
  PUBE: "Pube finance",
  AWF: "Alphawolf Finance",
  YCC: "Yuan Chain Coin",
  SPDR: "SpiderDAO",
  BNS: "BNS Token",
  DCH: "DeChart",
  SUP: "SUP",
  KIF: "KittenFinance",
  CWAP: "DeFIRE",
  BDO: "bDollar",
  BANANA: "ApeSwap Finance",
  BAFI: "Bafi Finance",
  REAP: "ReapChain",
  NSFW: "xxxNifty",
  BCHBEAR: "3x Short Bitcoin Cash Token",
  GUSDT: "Global Utility Smart Digital Token",
  HPX: "HUPAYX",
  CARR: "Carnomaly",
  XRPBEAR: "3x Short XRP Token",
  EFK: "ReFork",
  DVG: "DAOventures",
  DPET: "My DeFi Pet",
  LINKBEAR: "3X Short Chainlink Token",
  RAKU: "RAKUN",
  CNX: "Cofinex Coin",
  SHOPX: "Splyt",
  EBOX: "Ethbox",
  GME: "GameStop tokenized stock FTX",
  WSOTE: "Soteria",
  SHFT: "Shyft Network",
  LCG: "LCG",
  BVND: "Binance VND",
  DWS: "DWS",
  RUSH: "RUSH COIN",
  UTU: "UTU Protocol",
  SPA: "Sperax",
  LNCHX: "LaunchX",
  ROPE: "ROPE Token",
  HUA: "Chihuahua",
  SMOON: "SaylorMoon",
  MYID: "MY IDENTITY COIN",
  ALOHA: "Aloha",
  ZEFI: "ZCore Finance",
  GNT: "GreenTrust",
  HOTDOGE: "HotDoge",
  DFND: "dFund",
  SMBSWAP: "Simbcoin Swap",
  GDT: "Gorilla Diamond",
  AIN: "AI Network",
  PWAY: "PWAY",
  DAILY: "Daily",
  NDAU: "Ndau",
  SAFEBTC: "SafeBTC",
  CAN: "Coinwaycoin",
  PHIBA: "Papa Shiba",
  AQUA: "Planet Finance",
  WELL: "WELL",
  WBX: "WiBX",
  TATA: "HakunaMatata",
  AFC: "Apiary Fund Coin",
  CLIQ: "DefiCliq",
  PREMIA: "Premia",
  JUS: "JUST NETWORK",
  YELD: "Yeld Finance",
  BLES: "Blind Boxes",
  L2P: "Lung Protocol",
  FOMO: "FOMO LAB",
  CBAT: "Compound Basic Attention Token",
  SPE: "Save Planet Earth",
  HINT: "Hintchain",
  DMUSK: "Dragonmusk",
  CNFI: "Connect Financial",
  SBF: "SteakBankFinance",
  DEEZ: "deeznuts",
  PIE: "DeFiPie",
  JNTR: "Jointer",
  FAST: "FastSwap",
  CATZ: "CatzCoin",
  QRX: "QuiverX",
  PUSSY: "Pussy Financial",
  PRVS: "Previse",
  IIC: "Intelligent Investment Chain",
  DAPPT: "Dapp Token",
  XQC: "QURAS",
  MANY: "Manyswap",
  GHT: "Global Human Trust",
  DON: "DEONEX COIN",
  NZDX: "eToro New Zealand Dollar",
  SCONEX: "Sportcash One",
  USF: "Unslashed Finance",
  KEYT: "REBIT",
  UCO: "Uniris",
  ETHIX: "EthicHub",
  ALU: "Altura",
  XGG: "10x.gg",
  SWAPS: "NFTSwaps",
  "100X": "100xCoin",
  A5T: "Alpha5",
  BQTX: "BQT",
  XTK: "xToken",
  SAFESUN: "SAFESUN",
  DGCL: "DigiCol",
  ZYRO: "Zyro",
  LYFE: "LYFE",
  FULLSEND: "Full Send",
  CORGI: "Corgi inu",
  FC: "FansCoin",
  PANDO: "Pando",
  BITT: "BiTToken",
  CALI: "CaliCoin",
  METAMOON: "MetaMoon",
  BFLY: "Butterfly Protocol",
  CLS: "Coldstack",
  NUX: "Peanut",
  STOP: "Satopay Network",
  BU: "BUMO",
  WAULTX: "Wault [New]",
  BAS: "Basis Share",
  UNN: "UNION Protocol Governance Token",
  MIN: "MINDOL",
  MLT: "MILC Platform",
  NFI: "Norse Finance",
  LTCBULL: "3x Long Litecoin Token",
  CIX100: "Cryptoindex.com 100",
  NBU: "Nimbus",
  VIAGRA: "Viagra Token",
  TOK: "TOKOK",
  MOONI: "Mooni DeFi",
  REDI: "REDi",
  BUND: "Bundles Finance",
  ICE: "Popsicle Finance",
  CUBE: "Somnium Space Cubes",
  PEKC: "PEACOCKCOIN",
  DENA: "Decentralized Nations",
  EMON: "Ethermon",
  GMEE: "GAMEE",
  FONT: "Font",
  MOGX: "Mogu",
  SAFEBANK: "SafeBank YES",
  UC: "YouLive Coin",
  NGM: "e-Money",
  UNIC: "Unicly",
  DOGEMOON: "DogeMoon",
  QUAI: "QUAI DAO",
  SDAO: "SingularityDAO",
  GYEN: "GYEN",
  DOGEFATHER: "Dogefather",
  NMX: "Nominex Token",
  FRA: "Findora",
  MM: "MM Token",
  HBT: "Hubii Network",
  MEDIA: "Media Network",
  CLT: "CoinLoan",
  BLN: "Blacknet",
  MRCH: "MerchDAO",
  STACK: "Stacker Ventures",
  DEGEN: "DEGEN Index",
  OBOT: "OBORTECH",
  BLX: "Balloon-X",
  EA: "EA Token",
  LBXC: "Lux Bio Cell",
  BSBT: "BSB Token",
  DXF: "Dexfin",
  COLD: "COLD FINANCE",
  AUR: "Auroracoin",
  BSE: "Buy-Sell",
  mGOOGL: "Mirrored Google",
  RAK: "Rake Finance",
  MT: "MyToken",
  erowan: "SifChain",
  SAFEEARTH: "SafeEarth",
  BUP: "BuildUp",
  CROSS: "CrossPad",
  PSWAP: "Polkaswap",
  MTR: "Meter Stable",
  SOVI: "Sovi Finance",
  EXEN: "Exen Coin",
  SKYLARK: "Rocket Moon GO",
  MASH: "Marshmallowdefi",
  VRGX: "VROOMGO",
  USHIBA: "American Shiba",
  BAMBOO: "BambooDeFi",
  SAFEMUSK: "SafeMusk",
  FEED: "FEED Token",
  BTSUCN: "BTSunicorn",
  ORO: "ORO",
  SPORE: "Enoki Finance",
  POWER: "UniPower",
  RIT20: "Uberstate RIT 2.0",
  ARCONA: "Arcona",
  HIBS: "Hiblocks",
  DOGIRA: "Dogira",
  GAINS: "Gains Associates",
  TSX: "TradeStars",
  DINGO: "DINGO TOKEN",
  WCELO: "Wrapped Celo",
  CLV: "Clover",
  HTB: "Hotbit Token",
  UNW: "UniWorld",
  TOOLS: "BSC TOOLS",
  CRING: "Darwinia Crab Network",
  SYA: "Save Your Assets",
  GOVI: "Govi",
  LTN: "Life Token",
  GLDY: "Buzzshow",
  B20: "B20",
  FSXU: "FlashX Ultra",
  BPX: "Bispex",
  CC: "CCSwap",
  TSLA: "Tesla tokenized stock Bittrex",
  HZT: "Black Diamond Rating",
  SAFEGALAXY: "SafeGalaxy",
  BILI: "Billibilli tokenized stock Bittrex",
  BUSY: "Busy DAO",
  FOX: "FOX Token",
  SMRAT: "Secured MoonRat Token",
  PPDEX: "Pepedex",
  RENFIL: "renFIL",
  GBK: "Goldblock",
  BNOX: "BlockNoteX",
  SHIBACASH: "ShibaCash",
  KEK: "CryptoKek",
  ITS: "IterationSyndicate",
  FFF: "Force For Fast",
  WGRT: "WaykiChain Governance Coin",
  BKBT: "BeeKan",
  FVT: "Finance.Vote",
  XKI: "Ki",
  SI: "Siren",
  MIS: "Mithril Share",
  AXN: "Axion",
  LOON: "Loon Network",
  SETS: "Sensitrust",
  HMNG: "HummingBird Finance",
  KONJ: "KONJUNGATE",
  MEET: "CoinMeet",
  FL: "Freeliquid",
  SECO: "Serum Ecosystem Token",
  CPX: "Crypxie",
  TSM: "Taiwan Semiconductor Mfg tokenized stock FTX",
  B21: "B21 Invest",
  LIMIT: "LimitSwap",
  TRDG: "Tardigrades Finance",
  MUNCH: "Munch Token",
  EDGT: "Edgecoin",
  JSB: "JSB FOUNDATION",
  "POP!": "POP",
  DEXG: "Dextoken",
  DMC: "Decentralized Mining Exchange",
  PAND: "Panda Finance",
  EGCC: "Engine",
  MBS: "MoonBoys",
  PLE: "Plethori",
  FAI: "Fairum",
  VANCAT: "Vancat",
  OAP: "OpenAlexa Protocol",
  GMR: "GMR Finance",
  CROX: "CroxSwap",
  SLNV2: "StarLink",
  SHE: "ShineChain",
  imBTC: "The Tokenized Bitcoin",
  HBO: "HashBridge Oracle",
  LAVA: "LavaSwap",
  GHD: "Giftedhands",
  WHX: "WHITEX",
  SPICE: "Spice",
  GAZE: "GazeTV",
  BHIBA: "Baby Shiba",
  YAMV2: "YAMv2",
  LEV: "Lever Token",
  ETLT: "Ethereum Lightning",
  B360: "B360",
  VIDYX: "VidyX",
  MCO2: "Moss Carbon Credit",
  ETHBN: "EtherBone",
  HAPY: "HAPY Coin",
  HYMETEOR: "HyperMeteor",
  RAMEN: "RamenSwap",
  SPOK: "Spockchain Network",
  ALGOBULL: "3X Long Algorand Token",
  ZEP: "Zeppelin DAO",
  XMARK: "xMARK",
  TKING: "Tiger King",
  GSMT: "GrafSound",
  CHANGE: "Change Our World",
  VD: "VinDax Coin",
  WANUSDC: "wanUSDC",
  TRXBULL: "3X Long TRX Token",
  EDU: "EduCoin",
  SHARK: "Baby Shark",
  HOGL: "HOGL finance",
  CLAIM: "CLAIM",
  ELENA: "Elena Protocol",
  PLUM: "PlumCake Finance",
  HUSKY: "Husky",
  MoFi: "MobiFi",
  NCAT: "NCAT Token",
  EARN: "Yearn Classic Finance",
  BURN1: "Burn1 Coin",
  NEBO: "CSP DAO",
  ETGF: "ETG Finance",
  DT: "Dragon Token",
  PTERIA: "Pteria",
  CAP: "Capital.Finance",
  FMG: "FM Gallery",
  AQUARI: "Aquari",
  ORANGE: "Orange Token",
  YFIAG: "YearnAgnostic Finance",
  BBANK: "BlockBank",
  JACK: "Jack Token",
  TEAT: "TEAL",
  UBTC: "United Bitcoin",
  CBP: "CashBackPro",
  BAG: "Basis Gold",
  FLTY: "Fluity",
  EFT: "EFT.finance",
  UWL: "UniWhales",
  MSWAP: "MoneySwap",
  JBX: "JBOX",
  AKN: "Akoin",
  SHILD: "Shield Network",
  ATD: "A2DAO",
  PNODE: "Pinknode",
  WIS: "Experty Wisdom Token",
  LAS: "LNAsolution Coin",
  BTD: "Bolt Dollar",
  MOONTOKEN: "MoonToken",
  MVI: "Metaverse Index",
  SHIBAPUP: "ShibaPup",
  MIMO: "MIMOSA",
  FROGE: "Froge Finance",
  AENS: "AEN Smart Token",
  BULLS: "BULLS",
  COP: "Copiosa Coin",
  xDAI: "xDAI",
  TOZ: "TOZEX",
  VISION: "APY Vision",
  REL: "Relevant",
  PC: "Promotion Coin",
  MEDI: "MediconnectUk",
  DRGB: "Dragonbit",
  UNCL: "UNCL",
  MEL: "Melalie",
  SWOP: "Swop",
  GFARM2: "Gains Farm",
  SLF: "Solarfare",
  GZIL: "governance ZIL",
  CLOUT: "BLOCKCLOUT",
  RVF: "Rocket Vault",
  REFI: "Realfinance Network",
  CENT: "CENTERCOIN",
  EMB: "Emblem",
  ALTBULL: "3X Long Altcoin Index Token",
  MORA: "Meliora",
  DEEZNUTS: "Deez Nuts",
  TLB: "The Luxury Coin",
  ATTN: "ATTN",
  BSP: "Ballswap",
  SAFEP: "Safe Protocol",
  GEMS: "SafeGem.Finance",
  SDFI: "Sting Defi",
  TEST: "Test Token",
  GMNG: "Global Gaming",
  MDU: "MDUKEY",
  PAWS: "Animal Adoption Advocacy",
  JOYS: "Joys Digital",
  JNB: "Jinbi Token",
  UNIDX: "UniDex",
  SX: "SportX",
  AUSCM: "Auric Network",
  PINKM: "PinkMoon",
  DEXTF: "DEXTF Protocol",
  GMT: "Gambit",
  SAFESTAR: "Safe Star",
  GBTC: "Grayscale Bitcoin Trust tokenized stock FTX",
  PIPI: "Pippi Finance",
  SHX: "Stronghold Token",
  SASHIMI: "Sashimi",
  AICO: "AICON",
  ALPA: "Alpaca City",
  YEFIM: "YFi Management",
  PTD: "Pilot",
  SPACE: "Farm Space",
  DACC: "DACC",
  SATX: "SatoExchange Token",
  WHL: "WhaleRoom",
  BT: "BT.Finance",
  XFIT: "XFai",
  YFIM: "Yfi.mobi",
  SYL: "XSL Labs",
  RAPTOR: "Raptor Token",
  LOF: "Lonelyfans",
  X2P: "Xenon Pay II",
  KLP: "Kulupu",
  LABO: "The Lab Finance",
  ECP: "Eclipse",
  EIDOS: "EIDOS",
  JEM: "Itchiro Games",
  LAIKA: "LaikaCoin",
  SDOG: "Small dogecoin",
  XTZBULL: "3x Long Tezos Token",
  KP3RB: "Keep3r BSC Network",
  BRICK: "Brickchain Finance",
  THS: "The Hash Speed",
  HRD: "HRDCOIN",
  XPO: "Xpool",
  AFEN: "AFEN Blockchain",
  IDX: "Index Chain",
  RSV: "Reserve",
  PCI: "PayProtocol",
  MCH: "MEME CASH",
  ATOMBULL: "3X Long Cosmos Token",
  BINGUS: "Bingus Token",
  AAPX: "AMPnet Asset Platform and Exchange",
  SZC: "Zugacoin",
  BTSG: "BitSong",
  PYX: "PYXIS Network",
  ZWAP: "Zilswap",
  DGP: "DGPayment",
  MNTIS: "Mantis",
  MQL: "MiraQle",
  DONK: "Donkey",
  POODL: "Poodl Token",
  XCP: "Counterparty",
  ROZ: "Rozeus",
  GRILL: "Grill.Farm",
  DESH: "DeCash",
  GRUMPY: "Grumpy Finance",
  ELEPHANT: "Elephant Money",
  BBI: "BigBoys Industry",
  DCTD: "DCTDAO",
  FAR: "Farmland Protocol",
  CPI: "Crypto Price Index",
  GST: "The Gemstone",
  SAFEICARUS: "Safeicarus",
  MILK: "Milk Token",
  HPY: "Hyper Pay",
  PVM: "PRIVATEUM INITIATIVE",
  GYSR: "GYSR",
  GOAT: "GOAT COIN",
  LONG: "LONG COIN",
  BSKT: "BasketCoin",
  ibETH: "Interest Bearing ETH",
  XRGE: "RougeCoin",
  BCNT: "Bincentive",
  ICA: "Icarus Finance",
  SLAM: "Slam Token",
  ARTH: "ARTH",
  PBOM: "Pocket Bomb",
  MTTCOIN: "Money of Tomorrow, Today",
  SHARD: "Shard",
  MOONSTAR: "MoonStar",
  WGOLD: "APWars",
  TIGER: "Tigerfinance",
  BITO: "BitoPro Exchange Token",
  CHEX: "Chintai",
  WASABI: "WasabiX",
  BONE: "Bone",
  BUIDL: "DFOhub",
  PUNK: "Punk",
  ZUZ: "ZUZ Protocol",
  XAG: "Xrpalike Gene",
  CHFT: "Crypto Holding Frank Token",
  VLK: "Vulkania",
  JFC: "JFIN",
  MOVE: "Mover",
  JAGUAR: "JaguarSwap",
  DMX: "Dymmax",
  EHASH: "EHash",
  REAU: "Vira-lata Finance",
  WSB: "WallStreetBets DApp",
  REDPANDA: "Redpanda",
  LUD: "Ludos Protocol",
  LEMO: "LemoChain",
  LAYERX: "UNILAYERX",
  EARNX: "EarnX",
  SAFECOOKIE: "SAFECOOKIE",
  BCR: "Bankcoin Reserve",
  COCO: "Coco Swap",
  KGC: "Krypton Galaxy Coin",
  ASY: "ASYAGRO",
  DWZ: "DeFi Wizard",
  ELAND: "ETHERLAND",
  "X-TOKEN": "X-Token",
  UCAP: "Unicap.finance",
  BOTS: "BotOcean",
  ONES: "OneSwap DAO Token",
  PNGN: "SpacePenguin",
  GSPI: "GSPI Shopping.io Governance",
  RNB: "Rentible",
  STARSHIP: "STARSHIP",
  PFE: "Pfizer tokenized stock FTX",
  LTCBEAR: "3x Short Litecoin Token",
  SST: "SIMBA Storage Token",
  MNSTP: "Moon Stop",
  GPT: "Grace Period Token",
  POLVEN: "Polka Ventures",
  APES: "ApeHaven",
  ELX: "Energy Ledger",
  DOVE: "DoveSwap Finance",
  WENMOON: "WenMoon",
  TWEE: "Tweebaa",
  PIKA: "Pika",
  PATH: "PathFund",
  WAXE: "WAXE",
  TRXBEAR: "3X Short TRX Token",
  SBDO: "bDollar Share",
  LOCC: "Low Orbit Crypto Cannon",
  KEL: "KelVPN",
  COMFI: "CompliFi",
  CATX: "CAT.trade Protocol",
  UAT: "UltrAlpha",
  CE: "Crypto Excellence",
  PDAO: "Panda Dao",
  MOFI: "Mofi Finance",
  ARCC: "Asia Reserve Currency Coin",
  WLEO: "Wrapped LEO",
  UPR: "Upfire",
  DGVC: "DegenVC",
  iOWN: "iOWN Token",
  ONC: "One Cash",
  WDR: "Widercoin",
  FCH: "Fanaticos Cash",
  SFJP: "SafeJupiter $SFJP",
  BULLSHIT: "3X Long Shitcoin Index Token",
  DSYS: "DSYS",
  AGOLP: "AlgOil",
  EXCC: "ExchangeCoin",
  RAM: "Ramifi Protocol",
  SFUEL: "SparkPoint Fuel",
  LBY: "Libonomy",
  GDC: "Global Digital Content",
  ARGO: "ArGoApp",
  FWB: "Friends With Benefits",
  DOGETF: "Doge Father Token",
  CC10: "Cryptocurrency Top 10 Tokens Index",
  DOGZ: "Dogz",
  BVOL: "1x Long Bitcoin Implied Volatility Token",
  PROPEL: "PayRue (Propel)",
  SAFEZONE: "SafeZone",
  UPSHIB: "upShib",
  STRI: "Strite",
  HVE2: "Uhive",
  HYBN: "Hey Bitcoin",
  PAPEL: "Papel Token",
  KP2R: "KP2R.Network",
  NNB: "NNB Token",
  TOS: "ThingsOperatingSystem",
  MYO: "Mycro",
  TCAKE: "Tcake",
  LNT: "Lottonation",
  "1GOLD": "1irstGold",
  NDN: "NDN Link",
  TBTC: "tBTC",
  PAL: "PlayAndLike",
  CBSN: "BlockSwap Network",
  FOCV: "FOCV",
  CHAD: "CHAD",
  BNA: "Bananatok",
  JUR: "Jur",
  OGC: "One Get Coin",
  CLIMB: "CLIMB TOKEN FINANCE",
  YFIX: "YFIX Finance",
  XLMG: "Stellar Gold",
  KIWI: "Kiwi Finance",
  DILI: "D Community",
  RX: "Raven X",
  FRMX: "FRMx Token",
  GOLDUCK: "Golden Duck",
  POOCOIN: "PooCoin",
  UPDOG: "UPDOG",
  KFC: "Chicken",
  WENLAMBO: "Wenlambo",
  VBSWAP: "vBSWAP",
  LTP: "LifetionCoin",
  RELI: "Relite Finance",
  BNSD: "BNSD Finance",
  SCORGI: "SpaceCorgi",
  CHORD: "Chord Protocol",
  YT: "Cherry Token",
  DIGI: "Digible",
  GIX: "GoldFinX",
  CBT: "Community Business Token",
  GOGO: "GOGO.finance",
  RENZEC: "renZEC",
  FTF: "FantasyTurf",
  BUILD: "BUILD Finance",
  TEM: "Temtum",
  SAFELIGHT: "SafeLight",
  PASTA: "Pasta Finance",
  KBTC: "Klondike BTC",
  JWL: "Jewel",
  ORBYT: "ORBYT Token",
  RISKMOON: "RiskMoon",
  IFEX: "Interfinex",
  MTCN: "Multicoin",
  NUDEZ: "NudezCoin",
  RUC: "RUSH",
  HEBE: "HebeBlock",
  CRT: "Carrot",
  BB: "BlackBerry tokenized stock FTX",
  RWN: "Rowan Token",
  SAMO: "Samoyedcoin",
  RAP: "REALPAY",
  MCHC: "My Crypto Heroes",
  BUT: "BitUP Token",
  INTX: "intexcoin",
  MDO: "Midas Dollar",
  UBER: "Uber tokenized stock FTX",
  ATOMBEAR: "3X Short Cosmos Token",
  SAP: "SwapAll",
  GLDR: "GolderGames",
  BNZ: "BonezYard",
  SRSB: "Sirius Bond",
  ALLEY: "NFT Alley",
  SNOWGE: "SnowgeCoin",
  SIMPLE: "SimpleChain",
  TCO: "Tcoin",
  ETNXP: "Electronero Pulse",
  LVX: "Level01",
  ZTNZ: "Ztranzit Coin",
  NAN: "NanTrade",
  USDP: "USDP Stablecoin",
  DAILYS: "DailySwap Token",
  ONS: "One Share",
  RVX: "Rivex",
  VSO: "Verso Token",
  VNTW: "Value Network",
  TESLF: "Teslafan",
  SPERM: "Elon Sperm",
  XEQ: "Equilibria",
  GROW: "GrowingFi",
  GGT: "GARD Governance Token",
  GOFI: "GoSwapp",
  JDC: "JD Coin",
  BIST: "Bistroo",
  BSVBULL: "3x Long Bitcoin SV Token",
  SAFECOMET: "SafeComet",
  NFTFY: "Nftfy",
  ROVER: "Rover Inu Token",
  KODX: "KING OF DEFI",
  MINI: "MiniSwap",
  HYPER: "HyperChain",
  YFOX: "YFOX FINANCE",
  MUTE: "Mute",
  GAPT: "Gaptt",
  LID: "Liquidity Dividends Protocol",
  SUNI: "SUNI",
  YUI: "YUI Token",
  $COIN: "COIN",
  HEDGE: "1x Short Bitcoin Token",
  CHM: "Cryptochrome",
  PORTAL: "Portal",
  VVT: "VersoView",
  ECTE: "EurocoinToken",
  KLT: "KLend",
  MAZ: "Mazzuma",
  ORFANO: "Orfano",
  BKKG: "BIOKKOIN",
  LOA: "LOA Protocol",
  ACXT: "ACDX Exchange Governance Token",
  BLOSM: "BlossomCoin",
  CSPC: "CSPC",
  ESTI: "Easticoin",
  NTB: "TokenAsset",
  ETCH: "ElonTech",
  YLFI: "Yearn Loans Finance",
  DEVA: "DEVA TOKEN",
  TYC: "Tycoon",
  "yVault LP-yCurve(YYCRV)": "yVault LP-yCurve",
  HUNGRY: "Hungry Bear",
  XDAG: "XDAG",
  SPY: "SPDR S&P 500 ETF tokenized stock Bittrex",
  MOONMOON: "MoonMoon",
  GOOGL: "Google tokenized stock Bittrex",
  DNS: "BitDNS",
  F1C: "Future1coin",
  PYPL: "PayPal tokenized stock FTX",
  ETM: "En-Tan-Mo",
  HEJJ: "HEDGE4.Ai",
  BCEO: "bitCEO",
  DIAMOND: "DiamondToken",
  MOCHI: "MOCHISWAP",
  MKCY: "Markaccy",
  WHIRL: "Whirl Finance",
  OCTA: "Octans",
  OBR: "Order of the Black Rose",
  ARTX: "ARTX Trading",
  LDN: "Ludena Protocol",
  PRY: "Prophecy",
  XPB: "Transmute Protocol",
  BLOWF: "BlowFish",
  LMCH: "Latamcash",
  FRY: "Foundry",
  MNDAO: "MoonDAO",
  AMC: "AMC Entertainment Holdings tokenized stock FTX",
  BUTTER: "Butter TOken",
  ADABEAR: "3X Short Cardano Token",
  VXV: "Vectorspace AI",
  YOT: "PayYoda",
  UHP: "Ulgen Hash Power",
  MRS: "Marsan Exchange token",
  ODIN: "ODIN PROTOCOL",
  NVA: "Neeva Defi",
  TACOCAT: "TacoCat",
  PLG: "Pledge Coin",
  VIKING: "Viking Swap",
  SBEAR: "yBEARSwap",
  TRYB: "BiLira",
  RPT: "Rug Proof",
  DSS: "Defi Shopping Stake",
  WAYF: "WAY-F coin",
  TFC: "Triforce Protocol",
  LOT: "Lukki Operating Token",
  BVL: "Bullswap Exchange",
  MXW: "Maxonrow",
  RAFF: "Rafflection",
  PTM: "Potentiam",
  TV: "Ti-Value",
  MERCI: "MERCI",
  DRGNBULL: "3X Long Dragon Index Token",
  WSCRT: "Secret (ERC20)",
  UTED: "United",
  ELNC: "EloniumCoin",
  "808TA": "808TA",
  YFD: "Your Finance Decentralized",
  "420X": "420x",
  XGT: "Xion Finance",
  PROT: "Prostarter",
  DEXM: "DexMex",
  LEM: "Lemur Finance",
  XCF: "Cenfura Token",
  PLUTO: "PlutoPepe",
  GR: "Grom",
  SKI: "Skillchain",
  XDEX: "xDeFi",
  KAU: "Kauri",
  XTZBEAR: "3x Short Tezos Token",
  VINCI: "Vinci",
  TMED: "MDsquare",
  OKBBULL: "3X Long OKB Token",
  EMT: "Emanate",
  GM: "GM Holding",
  TERN: "Ternio",
  LUXO: "LUXOCHAIN",
  YPIE: "PieDAO Yearn Ecosystem Pie",
  SWAMP: "Swampy",
  POFI: "Pofi",
  DSCPL: "DISCIPLINA",
  BBKFI: "BitBlocks Finance",
  BSDS: "Basis Dollar Share",
  WDC: "WorldCoin",
  OTB: "OTCBTC Token",
  PAYB: "Paybswap",
  MIDBULL: "3X Long Midcap Index Token",
  GNSH: "Ganesha Token",
  UNDG: "UniDexGas",
  DEFIT: "Digital Fitness",
  USDL: "USDL",
  QHC: "QChi Chain",
  DSD: "DeFi Nation Signals DAO",
  TBT: "TideBit Token",
  ALGOBEAR: "3X Short Algorand Token",
  SXI: "SafeXI",
  DFIO: "DeFi Omega",
  HBD: "Hive Dollar",
  FSAFE: "Fair Safe",
  DELTA: "Delta",
  BSY: "Bestay",
  NEC: "Nectar",
  HAMTARO: "Hamtaro",
  DJ15: "Davincij15 Token",
  POLT: "Polkatrain",
  GC: "Gric Coin",
  BSHIBA: "Shiba Corp",
  KET: "Rowket",
  DISC: "DisCas Vision",
  BSVBEAR: "3x Short Bitcoin SV Token",
  LKM: "Lokum Finance",
  KRYPTO: "Kryptobellion",
  IOEX: "IOEX",
  SKILL: "CryptoBlades",
  BFI: "BFis.Finance",
  EYE: "Behodler",
  MEX: "MEX",
  BRN: "Brainaut Defi",
  ANI: "Anime Token",
  AGRS: "Agoras Tokens",
  NVDA: "NVIDIA tokenized stock FTX",
  ARA: "Ara Blocks",
  CHIK: "Chickenkebab Finance",
  KODURO: "Koduro",
  YNI: "YEARNYFI NETWORK",
  FOMP: "FOMPOUND",
  ALIAS: "Alias",
  QUAM: "Quam Network",
  GVE: "Globalvillage Ecosystem",
  MINDS: "Minds",
  XEP: "Electra Protocol",
  GUAP: "Guapcoin",
  DOOS: "DOOS TOKEN",
  JULB: "JustLiquidity Binance",
  CIR: "CircleSwap",
  MWG: "Metawhale Gold",
  D100: "DeFi100",
  ISAL: "ISALCOIN",
  LIMEX: "Limestone Network",
  FLS: "Flits",
  SBT: "SOLBIT",
  SAFU: "CEEZEE SAFU",
  JDB: "Justdobet",
  ROOT: "Rootkit Finance",
  "4ART": "4ART Coin",
  JINDOGE: "Jindoge",
  ARIGATO: "Arigato",
  ZOC: "01coin",
  SHOKK: "Shikokuaido",
  YOK: "YOKcoin",
  GGTK: "GGDApp",
  R3FI: "Recharge Finance",
  CARBO: "CarbonDEFI Finance",
  METRIC: "Metric Exchange",
  "DEFI+L": "PieDAO DEFI Large Cap",
  IQQ: "IQONIQ FanEcoSystem",
  SOGE: "Space Hoge",
  bPRIVA: "Privapp Network",
  VOICE: "Voice Token",
  SAFEORBIT: "SafeOrbit",
  NIU: "Niubi Swap",
  RARE: "Unique One",
  SUP8EME: "SUP8EME",
  GASP: "gAsp",
  PSI: "Passive Income",
  SLD: "SafeLaunchpad",
  YFFS: "YFFS Finance",
  EDI: "Freight Trust & Clearing Network",
  GOM: "Gomics",
  PLBT: "Polybius",
  CIPHC: "Cipher Core Token",
  EXF: "Extend Finance",
  SHEESHA: "Sheesha Finance [ERC20]",
  COMFY: "ComfyToken",
  HPS: "HappinessToken",
  TACO: "Tacoswap",
  UTNP: "Universa",
  "DEFI+S": "PieDAO DEFI Small Cap",
  ROBO: "Robo Token",
  FRIES: "fry.world",
  MUST: "Cometh",
  ESPRO: "EsportsPro",
  SUSHIBEAR: "3X Short Sushi Token",
  NANA: "Ape Tools",
  YFTE: "YFTether",
  FLEX: "FLEX",
  TKMN: "Tokemon",
  CAMP: "Camp",
  VRO: "VeraOne",
  FOXD: "FoxDcoin",
  UMBR: "Umbria Network",
  AXIA: "Axia Protocol",
  ASP: "Aspire",
  ENV: "ENV Finance",
  HBC: "HBTC Captain Token",
  AWG: "AurusGOLD",
  BTYM: "Blocktyme",
  FBT: "FANBI TOKEN",
  PINKE: "PinkElon",
  FN: "Filenet",
  FX1: "FANZY",
  DGN: "Degen Protocol",
  TWI: "Trade.win",
  TCR: "TecraCoin",
  AIB: "Advanced Internet Blocks",
  VBK: "VeriBlock",
  BALI: "Bali Coin",
  IZE: "IZE",
  PRISM: "Prism Network",
  ABS: "Absorber Protocol",
  OVI: "Oviex",
  BDOG: "Bulldog Token",
  WAULT: "Wault Finance (OLD)",
  UME: "UME Token",
  CORAL: "Coral Swap",
  LUCK: "Lady Luck",
  DIC: "Daikicoin",
  PTA: "Petrachor",
  HPAY: "Hyper Credit Network",
  ARTEON: "Arteon",
  N3RDz: "N3RD Finance",
  ORCL5: "Oracle Top 5 Tokens Index",
  TOMOBULL: "3X Long TomoChain Token",
  PNIX: "PhoenixDefi.Finance",
  UBIN: "Ubiner",
  AIRX: "Aircoins",
  CASH: "Litecash",
  SCAT: "Sad Cat Token",
  NPXS: "Pundi X[old]",
  NLC2: "NoLimitCoin",
  FROG: "CryptoFrog.Finance",
  HUB: "Hub - Human Trust Protocol",
  DXC: "Dexchain",
  RMOON: "RocketMoon",
  CRS: "Crypto Rewards Studio",
  UFO: "Unknown Fair Object",
  YFBTC: "YFBitcoin",
  ETO: "Essek Tov",
  AVS: "AlgoVest",
  KIAN: "Kianite Finance",
  XVIX: "XVIX",
  VBZRX: "bZx Vesting Token",
  MSC: "Monster Slayer Cash",
  ZIK: "Ziktalk",
  ETHPY: "Etherpay",
  TBB: "Trade Butler Bot",
  PMP: "Pumpy farm",
  ACC: "Asian-African Capital Chain",
  PTN: "PalletOne",
  DND: "Dungeonswap",
  HTBULL: "3X Long Huobi Token Token",
  BNBBEAR: "3X Short BNB Token",
  "1337": "E1337",
  CARAT: "CARAT",
  EULER: "Euler Tools",
  NFTL: "NFTL Token",
  MODEX: "Modex",
  SPIZ: "SPACE-iZ",
  APLP: "Apple Finance",
  BNBD: "BNB Diamond",
  WDP: "WaterDrop",
  NTON: "NTON",
  TSR: "Tesra",
  AMI: "AMMYI Coin",
  SAFETESLA: "Safetesla",
  PERX: "PeerEx",
  YFS: "YFS.FINANCE",
  AK12: "AK12",
  "BTC++": "PieDAO BTC++",
  DICE: "TRONbetDice",
  SCV: "Super CoinView Token",
  RYIP: "RYI Platinum",
  SPANK: "SpankChain",
  ARKK: "ARK Innovation ETF tokenized stock FTX",
  MDAO: "Martian DAO",
  BOOZE: "BoozeMoon",
  SOME: "Mixsome",
  ETHHEDGE: "1X Short Ethereum Token",
  EGC: "EcoG9coin",
  STACY: "Stacy",
  TUSC: "The Universal Settlement Coin",
  UCT: "Ubique Chain Of Things",
  MP3: "MP3",
  WQT: "WorkQuest",
  TLW: "TILWIKI",
  TROP: "Interop",
  MKB: "Maker Basic-MKB",
  GYA: "GYA",
  COLX: "ColossusXT",
  GIV: "GIVLY Coin",
  DSWAP: "Definex",
  XAUTBULL: "3X Long Tether Gold Token",
  PIS: "PiSwap Token",
  PFID: "Pofid Dao",
  NDS: "Nodeseeds",
  MAGI: "Magikarp Finance",
  RECH: "ReChain.Finance",
  POOROCKET: "POOROCKET",
  yPANDA: "YieldPanda Finance",
  PWC: "Prime Whiterock Company",
  HP: "HeartBout Pay",
  SAFERUNE: "Saferune",
  ESRC: "ESR Coin",
  FOTO: "Unique Photo",
  UNIFI: "UNIFI DeFi",
  RANK: "Rank Token",
  ZYX: "ZYX",
  AKM: "COST COIN+",
  dART: "dART Insurance",
  WISB: "Wise Token",
  GCAT: "GingerCat",
  BRZX: "Braziliex Token",
  SURF: "SURF Finance",
  LKK: "Lykke",
  SWIRL: "Swirl Cash",
  ECHO: "Token Echo",
  AZX: "AzeusX",
  WINDY: "WindSwap",
  AURUM: "Alchemist DeFi Aurum",
  SBYTE: "Securabyte Protocol",
  GLOX: "Glox Finance",
  ARRO: "Arro Social",
  GLC: "Goldcoin",
  TMT: "Tamy Token",
  WAF: "Waffle",
  PUML: "PUML Better Health",
  B26: "B26 Finance",
  DEFLCT: "Deflect",
  USDFL: "USDFreeLiquidity",
  DEOR: "DEOR",
  EHRT: "Eight Hours",
  SNBL: "Nebulaprotocol",
  AWX: "AurusDeFi",
  DIGEX: "Digex",
  CHAR: "Charitas",
  FMA: "FLAMA",
  LEVELG: "LEVELG",
  SLM: "Solomon Defi",
  SWISS: "swiss.finance",
  XBN: "Elastic BNB",
  FMT: "Finminity",
  SAM: "Samurai",
  FDR: "French Digital Reserve",
  SENSE: "Sense",
  MWBTC: "MetaWhale BTC",
  XANK: "Xank",
  WCCX: "Wrapped Conceal",
  LUNAR: "Lunar Highway",
  ZGOAT: "GOAT Zuckerberg",
  MANGO: "Mango Finance",
  DEVE: "Divert Finance",
  SAV3: "Sav3Token",
  TNDR: "ThunderSwap",
  ABNB: "Airbnb tokenized stock FTX",
  DUSD: "DefiDollar",
  VCCO: "Vera Cruz Coin",
  LLT: "LifeLine",
  DION: "Dionpay",
  ETCBEAR: "3X Short Ethereum Classic Token",
  HXN: "Havens Nook",
  VSD: "Value Set Dollar",
  DEFI: "Defi",
  ECOIN: "Ecoin",
  RYIU: "RYI Unity",
  EOX: "EOX",
  XDEF2: "Xdef Finance",
  SAFECOM: "SAFE Community Token",
  DCY: "Dinastycoin",
  NAMI: "Tsunami finance",
  BRI: "Baroin",
  SCHO: "Scholarship Coin",
  SISHI: "Sishi Finance",
  MTF: "Milktea.finance",
  SWFL: "Swapfolio",
  VNDC: "VNDC",
  xBLZD: "Blizzard.money",
  CLA: "Candela Coin",
  DROPS: "DefiDrop Launchpad",
  LKN: "LinkCoin Token",
  AMD: "Advanced Micro Devices tokenized stock FTX",
  MMAON: "MMAON",
  WHEN: "WHEN Token",
  YUP: "Yup",
  JPYC: "JPYC",
  BNP: "BenePit Protocol",
  "777": "Jackpot",
  ALGOP: "AlgoPainter",
  LOV: "The LoveChain",
  CAROM: "Carillonium finance",
  MORK: "MORK",
  BABA: "Alibaba tokenized stock Bittrex",
  BNFY: "B Non-Fungible Yearn",
  NEU: "Neumark",
  STONK: "STONK",
  NBR: "Niobio",
  ZOM: "ZOM",
  FED: "Fedora Gold",
  PINT: "Pub Finance",
  KOIN: "Koinos",
  SLV: "Silverway",
  CL: "Coinlancer",
  YSR: "Ystar",
  YFIE: "YFIEXCHANGE.FINANCE",
  LIBFX: "Libfx",
  YAPE: "GorillaYield",
  BAEPAY: "BAEPAY",
  VMR: "VOMER",
  ZIPT: "Zippie",
  TTX: "Talent Token",
  EXRT: "EXRT Network",
  HAUS: "DAOhaus",
  PLUT: "Pluto",
  ODE: "ODE",
  XNODE: "XNODE",
  VRAP: "VeraSwap",
  DFX: "DeFireX",
  GIGA: "GigaPool",
  "50X": "50x.com",
  ENTONE: "ENTONE",
  MELLO: "Mello Token",
  JT: "Jubi Token",
  VANY: "Vanywhere",
  PXT: "Populous XBRL Token",
  AURO: "Aurora Finance",
  PSL: "Pastel",
  UTT: "United Traders Token",
  KONG: "Kong Defi",
  BIXB: "BIXBCOIN",
  POC: "POC Blockchain",
  CTF: "CyberTime Finance Token",
  FR: "Freedom Reserve",
  UOP: "Utopia Genesis Foundation",
  CSM: "Consentium",
  KIM: "KingMoney",
  INFINITY: "Infinity Protocol",
  "1ST": "FirstBlood",
  mFB: "Mirrored Facebook Inc",
  SMDX: "SOMIDAX",
  GSE: "GSENetwork",
  HITX: "Hithotx",
  BBGC: "Big Bang Game Coin",
  SAFT: "Sage Finance",
  D4RK: "Dark",
  AAPL: "Apple tokenized stock FTX",
  ESWAP: "eSwapping",
  FME: "FME",
  TOSHI: "Toshimon",
  SWAG: "SWAG Finance",
  YFW: "YFWorld",
  BA: "BaTorrent",
  NIO: "Nio tokenized stock FTX",
  BXK: "Bitbook Gambling",
  MYTV: "MyTVchain",
  ALG: "Bitalgo",
  FETA: "FetaToken",
  VTUBE: "VTube Token",
  KIMOCHI: "Kimochi Finance",
  KPC: "Koloop Basic",
  NYAN: "YieldNyan",
  PHOON: "Typhoon Cash",
  QAC: "Quasarcoin",
  MSS: "Monster Slayer Share",
  PARTY: "MONEY PARTY",
  DOGDEFI: "DogDeFiCoin",
  YFIS: "YFISCURITY",
  MIDAS: "Midas",
  SPRK: "Sparkster",
  YNK: "Yoink",
  HBDC: "happy birthday coin",
  URAC: "Uranus",
  IDO: "IDOHunt app",
  XFUEL: "XFUEL",
  YFIEC: "Yearn Finance Ecosystem",
  COFE: "CoffeeSwap",
  MYC: "Myteamcoin",
  KIP: "Khipu Token",
  SOP: "SoPay",
  IPM: "Timers",
  XXA: "Ixinium",
  RUPEE: "HyruleSwap",
  BDOGE: "Blue Eyes White Doge",
  STP: "STPAY",
  C2O: "CryptoWater",
  DMS: "Documentchain",
  BASK: "BasketDAO",
  ADL: "Adelphoi",
  POLAR: "Polar",
  VYBE: "Vybe",
  DEEP: "DeepCloud AI",
  HTA: "Historia",
  A2A: "A2A",
  VDX: "Vodi X",
  MNS: "MONNOS",
  BCDN: "BlockCDN",
  EMRALS: "Emrals",
  APE: "Apestrong finance",
  DFGL: "DeFi Gold",
  ACU: "Aitheon",
  MLR: "Mega Lottery Services Global",
  MANDI: "Mandi Token",
  PQT: "PREDIQT",
  FECLIPSE: "FairEclipse",
  DETS: "Dextrust",
  CCE: "CloudCoin",
  DINU: "Dogey-Inu",
  YEA: "YeaFinance",
  PSIX: "ProperSix",
  YFN: "Yearn Finance Network",
  UNII: "UNII Finance",
  THX: "ThoreNext",
  GGC: "Global Game Coin",
  TAT: "Tatcoin",
  BLOOD: "Blood",
  BXA: "Blockchain Exchange Alliance",
  "NYAN-2": "Nyan V2",
  GNC: "GALAXY NETWORK",
  RIFT: "RIFT Token",
  NS: "Nodestats",
  LIQUID: "LIQUID",
  WXMR: "Wrapped Monero",
  CYF: "CY Finance",
  POX: "Pollux Coin",
  CETH: "Compound Ether",
  SPORTS: "ZenSports",
  NGC: "NAGA",
  PFI: "protocol finance",
  SNO: "SaveNode",
  YFIIG: "YFII Gold",
  SENPAI: "SENPAI",
  YFIA: "YFIA",
  IDH: "indaHash",
  DAFT: "DaftCoin",
  ZCK: "Polkazeck",
  KOMET: "Komet",
  LIGHT: "Lightning",
  TRAT: "Tratok",
  CRL: "CoralFarm",
  MEMES: "Memes Token",
  CREP: "Compound Augur",
  CROW: "Crow Finance",
  MAG: "Maggie",
  CWBTC: "Compound Wrapped BTC",
  TRND: "Trendering",
  BTAP: "BTA Protocol",
  FLUNAR: "FairLunar",
  GANJA: "trees.finance",
  MP4: "MP4",
  BPP: "Bitpower",
  CPTE: "Crypto Puzzles ERC20",
  XRUNE: "Rune",
  EZY: "EzyStayz",
  PAPER: "Fomo App",
  DIESEL: "DIESEL",
  AMZN: "Amazon tokenized stock FTX",
  IMO: "IMO",
  UCR: "Ultra Clear",
  GCX: "GermanCoin",
  AMZ: "AmazonasCoin",
  BLFI: "BlackFisk",
  MARSM: "MarsMission Protocol",
  SPHR: "Sphere",
  POKE: "Pokeball",
  DMME: "DMme",
  DEBASE: "Debase",
  MOONDAY: "Moonday Finance",
  ECOREAL: "Ecoreal Estate",
  MCRN: "MacaronSwap",
  IDALL: "IDall",
  BCU: "Biscuit Farm Finance",
  NFLX: "Netflix tokenized stock FTX",
  QOOB: "QOOBER",
  ZHEGIC: "zHEGIC",
  ITAM: "ITAM Games",
  TAO: "Friction Finance",
  SATVG: "SaturnV Gold",
  EFG: "ECOC Financial Growth",
  FCD: "Future-Cash Digital",
  SHD: "Shadetech",
  PDO: "Pollo Dollar",
  DDR: "DigiDinar",
  VLAD: "Vlad Finance",
  CYFM: "CyberFM",
  ILK: "INLOCK",
  IFUND: "Unifund",
  GFI: "Gorilla-Fi",
  TEP: "Tepleton",
  TBP: "Tradebitpay",
  DSL: "DeadSoul",
  OPM: "Omega Protocol Money",
  MRNA: "Moderna tokenized stock FTX",
  NIA: "Nydronia",
  AWS: "AurusSILVER",
  TGIC: "The global index chain",
  BHIG: "BuckHathCoin",
  WHEAT: "WHEAT Token",
  NDSK: "Nadeshiko",
  OIL: "Crudeoil Finance",
  UFFYI: "Unlimited FiscusFYI",
  BFF: "Bitcoffeen",
  KEN: "Keysians Network",
  AGVC: "AgaveCoin",
  XTRM: "XTRM COIN",
  GROOT: "growth Root Token",
  FSHN: "Fashion Coin",
  TME: "TAMA EGG NiftyGotchi",
  SCU: "Securypto",
  COMOS: "COMOS Finance",
  "DELTA-RLP": "Rebasing Liquidity",
  LUCY: "LUCY",
  WENR: "Weiner Finance",
  DVT: "DeVault",
  SLS: "SaluS",
  ITEN: "ITEN",
  PRDZ: "Predictz",
  OCB: "BLOCKMAX",
  SHRMP: "Shrimp Capital",
  STOGE: "Stoner Doge Finance",
  COIL: "COIL",
  EDS: "Endorsit",
  OX: "OrcaX",
  VSN: "Vision",
  YFET: "YFET",
  BYND: "Beyond Meat Inc tokenized stock Bittrex",
  TRIB: "Contribute",
  DEGOV: "Degov",
  WANBTC: "wanBTC",
  NUT: "Native Utility Token",
  PWRB: "PowerBalt",
  BEE: "BeeEx",
  KVA: "Kevacoin",
  SMOL: "Smol",
  SFN: "Strains Finance",
  OAK: "Octree",
  NARUTO2: "Naruto BSC",
  FRIDGE: "Fridge Token",
  MCDC: "McDonalds Coin",
  GLOB: "Global Reserve System",
  DDRT: "DigiDinar Token",
  VKF: "VKF Platform",
  ECOC: "ECOChain",
  RM: "Rivermount",
  TOR: "Torex",
  MXF: "Mixty Finance",
  ESK: "Eska",
  ME: "All.me",
  TRP: "Tronipay",
  ROX: "Robotina",
  ECC: "ECC",
  IDL: "IDL Token",
  DRAGON: "DragonFarm Finance",
  EXT: "Experience Token",
  TUNE: "TUNE TOKEN",
  ACPT: "Crypto Accept",
  TUDA: "Tutor's Diary",
  R2R: "CitiOs",
  TTC: "TTCRYPTO",
  ANK: "AlphaLink",
  SKC: "SKINCHAIN",
  FORS: "Foresight",
  SHIFT: "Shift",
  DPC: "DAPPCENTS",
  CAVO: "Excavo Finance",
  CREX: "Crex Token",
  SAFEYIELD: "SafeYield",
  NYC: "NewYorkCoin",
  CBUCKS: "CRYPTOBUCKS",
  RFCTR: "Reflector.Finance",
  TRI: "Trinity Protocol",
  FFYI: "Fiscus.fyi",
  YAS: "YAS",
  WIKI: "Wiki Token",
  HUE: "Hue",
  TWTR: "Twitter tokenized stock FTX",
  HATCH: "Hatch DAO",
  CT: "CommunityToken",
  FSC: "Five Star Coin",
  MSTR: "MicroStrategy tokenized stock FTX",
  JMC: "Junsonmingchncoin",
  BCHC: "BitCherry",
  WANLINK: "wanLINK",
  SOX: "Ethersocks",
  MTI: "MTI Finance",
  UPT: "Universal Protocol Token",
  EVR: "Everus",
  TMN: "TranslateMe Network Token",
  CO2B: "Co2B",
  BONO: "Bonorum",
  CHZ006: "Charizard Token",
  SWPRL: "Swaprol",
  DXH: "Daxhund",
  VEO: "Amoveo",
  BOLTT: "Boltt Coin",
  SMOKE: "The Smokehouse",
  ODEX: "One DEX",
  NEWW: "New Ventures",
  NBTC: "NEOBITCOIN",
  MACH: "MACH Project",
  PURE: "Puriever",
  BIUT: "Bit Trust System",
  USDB: "USD Bancor",
  BCNA: "BitCanna",
  METH: "Farming Bad",
  BSPAY: "Brosispay",
  SCH: "Schilling-Coin",
  COM: "Community Token",
  DEQ: "Dequant",
  EOC: "Everyonescrypto",
  MLA: "Moola",
  G9TRO: "g9tro Crowdfunding Platform",
  GASG: "Gasgains",
  WEBN: "WEBN token",
  LSV: "Litecoin SV",
  SPRINK: "SPRINK",
  FB: "Facebook tokenized stock FTX",
  WPX: "Wallet Plus X",
  ACDC: "Volt",
  WATER: "WaterDefi",
  VOLTZ: "Voltz",
  BLURT: "Blurt",
  SHPP: "Shipit pro",
  THE: "THENODE",
  ALA: "AladiEx",
  CHL: "ChallengeDac",
  THOR: "Asgard finance",
  BTCR: "Bitcurate",
  HOMT: "HOMT",
  ZCH: "0cash",
  DUN: "Dune Network",
  DFO: "DeFiato",
  BAK: "TokenBacon",
  SEND: "Social Send",
  $BASED: "Based Money",
  AET: "Aerotoken",
  "3Cs": "Crypto Cricket Club",
  FTB: "Free Tool Box Coin",
  WANEOS: "wanEOS",
  OUR: "Our Pay",
  DEGENS: "Degens",
  WSC: "WeSing Coin",
  VLU: "Valuto",
  HANA: "Hanacoin",
  BDCASH: "BigdataCash",
  HAZE: "Haze Finance",
  HUSL: "The Hustle App",
  NZO: "Enzo",
  GADOSHI: "Gadoshi",
  TKNT: "TKN Token",
  CHG: "Charg Coin",
  FXP: "FXPay",
  GOLDR: "Golden Ratio Coin",
  YFB2: "Yearn Finance Bit2",
  "7ADD": "HoldToWin",
  BPOP: "BPOP",
  YFFI: "yffi finance",
  NST: "Newsolution",
  TFD: "ETF Dao",
  HCA: "Harcomia",
  YMAX: "YMAX",
  ABET: "Altbet",
  LPS: "Lapis Chain",
  DVS: "Diamond Voucher",
  YFRM: "Yearn Finance Red Moon",
  SHENG: "SHENG",
  KMW: "Kepler Network",
  RICK: "Infinite Ricks",
  ECA: "Electra",
  YFI3: "YFI3.money",
  HLAND: "HLand Token",
  ZLP: "Zuplo",
  KAM: "BitKAM",
  MYX: "MYX Network",
  CDEX: "Codex",
  NOIZ: "NOIZ",
  VOLLAR: "V-Dimension",
  VJC: "VENJOCOIN",
  LYR: "Lyra",
  LIBREF: "LibreFreelencer",
  SQ: "Square tokenized stock FTX",
  FTO: "FuturoCoin",
  MGC: "MGC Token",
  BMCH: "bMeme Cash",
  DMTC: "Demeter Chain",
  DFIP: "DeFi Insurance Protocol",
  WLB: "Wallabee",
  IX: "X-Block",
  VANCII: "VANCI FINANCE",
  GTX: "GoalTime N",
  XFYI: "XCredit",
  SBE: "Sombe",
  USDQ: "USDQ",
  PCN: "PeepCoin",
  SORA: "SorachanCoin",
  KOMP: "Kompass",
  HIZ: "Hiz Finance",
  ECOS: "EcoDollar",
  RLE: "Richlab Token",
  PNIXS: "PhoenxiDefi Finance",
  ZUR: "Zurcoin",
  MAVRO: "Mavro",
  SCSX: "Secure Cash",
  YFA: "YFA Finance",
  TRXC: "TRONCLASSIC",
  DPT: "Diamond Platform Token",
  RC20: "RoboCalls",
  ETNX: "Electronero",
  SAVE: "SaveToken",
  VN: "VN Token",
  SEMI: "Semitoken",
  XEN: "Xenon",
  CMIT: "CMITCOIN",
  TST: "Touch Social",
  P2P: "P2P",
  SYM: "SymVerse",
  MACPO: "Master Coin Point",
  GETH: "Guarded Ether",
  SFR: "Safari",
  SGC: "Sudan Gold Coin",
  BTRL: "BitcoinRegular",
  YBEAR: "yBEAR.finance",
  CLBR: "Colibri Protocol",
  LVN: "LivenPay",
  DDAM: "Decentralized Data Assets Management",
  BCAPS: "Binacaps",
  PLUS1: "PlusOneCoin",
  CODEO: "Codeo Token",
  ANG: "Aureus Nummus Gold",
  YCE: "MYCE",
  B1P: "B ONE PAYMENT",
  TDPS: "Tradeplus",
  UCX: "UCX FOUNDATION",
  ROS: "ROS Coin",
  TORM: "Thorium",
  I9C: "i9 Coin",
  AZZR: "Azzure",
  NILU: "Nilu",
  WANSUSHI: "wanSUSHI",
  PROUD: "PROUD Money",
  HNDC: "HondaisCoin",
  TRISM: "Trism",
  HBN: "HoboNickels",
  HGH: "HGH Token",
  WANUNI: "wanUNI",
  IFLT: "InflationCoin",
  RBY: "Rubycoin",
  RBBT: "RabbitCoin",
  AfroX: "AfroDex",
  KBC: "Karatgold Coin",
  CLAM: "Clams",
  XQN: "Quotient",
  BUB: "Bubble",
  AXIOM: "Axiom",
  CLUB: "ClubCoin",
  AV: "AvatarCoin",
  FRN: "Francs",
  ACES: "Aces",
  BLAZR: "BlazerCoin",
  MSD: "MSD",
  EONS: "EONS",
  CFUN: "CFun",
  MOAC: "MOAC",
  IDXM: "IDEX Membership",
  OC: "OceanChain",
  VLC: "ValueChain",
  J8T: "JET8",
  FGC: "FantasyGold",
  CONI: "Coni",
  NEWOS: "NewsToken",
  INB: "Insight Chain",
  OBTC: "Obitan Chain",
  KXC: "KingXChain",
  YUKI: "YUKI",
  TAC: "Traceability Chain",
  GZE: "GazeCoin",
  MOLK: "MobilinkToken",
  DAGT: "Digital Asset Guarantee Token",
  WIX: "Wixlar",
  WIRE: "AirWire",
  DEEX: "DEEX",
  MEDIBIT: "MediBit",
  BGG: "Bgogo Token",
  OSA: "OSA Token",
  BRC: "Baer Chain",
  FIII: "Fiii",
  ROM: "ROMToken",
  TOSC: "T.OS",
  QUSD: "QUSD",
  AUNIT: "Aunite",
  HLT: "Esportbits",
  XTX: "Xtock",
  ATLS: "Atlas Token",
  NEAL: "Coineal Token",
  XSPC: "SpectreSecurityCoin",
  "7E": "7Eleven",
  MAYP: "Maya Preferred",
  QBX: "qiibee",
  EXOR: "EXOR",
  YTA: "YottaChain",
  GEX: "GLOBEX",
  SKYM: "SkyMap",
  ZDX: "Zer-Dex",
  OPNN: "Opennity",
  XENO: "Xenoverse",
  CUST: "Custody Token",
  LAD: "Ladder Network Token",
  BSOV: "BitcoinSoV",
  BTCX: "Bitscoin",
  KBOT: "Korbot",
  XSC: "xscoin",
  VENA: "VENA",
  LN: "LINK",
  SIERRA: "Sierracoin",
  CIPX: "Colletrix",
  TKX: "Tokenize Xchange",
  VERS: "Versess Coin",
  QPY: "QPay",
  XNS: "Xeonbit Token",
  VERA: "VERA",
  NOVA: "NOVA",
  DSC: "Dash Cash",
  BXT: "BITFXT COIN",
  MLGC: "Marshal Lion Group Coin",
  BITN: "Bitcoin & Company Network",
  BNKR: "Bankroll Network",
  XBG: "Bitgrin",
  SGR: "Sogur",
  USDA: "USDA",
  AXL: "Axial Entertainment Digital Asset",
  BCB: "Building Cities Beyond Blockchain",
  "1AI": "1AI Token",
  FRED: "FRED Energy",
  RES: "Resistance",
  KYF: "Kryptofranc",
  KTC: "Kratscoin",
  CSAI: "Compound SAI",
  ASAC: "Asac Coin",
  SSN: "SuperSkynet",
  EER: "Ethereum eRush",
  XLPG: "StellarPayGlobal",
  CELC: "CelCoin",
  CICX: "Cicoin",
  WOK: "weBloc",
  SWZL: "Swapzilla",
  PBTC: "pTokens BTC",
  ETF: "Entherfound",
  METP: "Metaprediction",
  FF1: "Two Prime FF1 Token",
  BKRW: "Binance KRW",
  MPS: "Mt Pelerin",
  TARM: "ARMTOKEN",
  ZCRT: "ZCore Token",
  VAIP: "VEHICLE DATA ARTIFICIAL INTELLIGENCE PLATFORM",
  CBET: "CryptoBet",
  "0xMR": "0xMonero",
  EYES: "EYES Protocol",
  BKS: "Barkis Network",
  UNIUSD: "UniDollar",
  QTV: "Quish Coin",
  BTCHG: "BITCOINHEDGE",
  IOOX: "IOOX System",
  ATUSD: "Aave TUSD",
  AKNC: "Aave KNC",
  ALINK: "Aave LINK",
  ASNX: "Aave SNX",
  AMKR: "Aave MKR",
  ABAT: "Aave BAT",
  ABUSD: "Aave BUSD",
  ADAI: "Aave DAI",
  SADA: "sADA",
  MHLX: "HelixNetwork",
  KUB: "Kublaicoin",
  TRCL: "Treecle",
  NOKN: "Nokencoin",
  NOLE: "NoleCoin",
  BPT: "Bitpumps Token",
  BLVR: "BELIEVER",
  SDEFI: "sDEFI",
  SXAG: "sXAG",
  KVNT: "KVANT",
  COL: "Unit Protocol",
  CBR: "Cybercoin",
  ZOOM: "CoinZoom",
  RWS: "Robonomics Web Services",
  UNC: "Unicrypt",
  MOV: "MOTIV Protocol",
  PUSD: "PegsUSD",
  KASH: "Kids Cash",
  CNYT: "CNY Tether",
  BWB: "Bit World Token",
  DGMT: "DigiMax DGMT",
  TOPB: "TOPBTC Token",
  CFT: "Coffee Token",
  KVI: "KVI",
  "YDAI+YUSDC+YUSDT+YTUSD": "LP-yCurve",
  "888": "888tron",
  MNR: "Mineral",
  BTMXBULL: "3X Long BitMax Token Token",
  DOGEBEAR: "3X Short Dogecoin Token",
  MATICBEAR: "3X Short Matic Token",
  TOMOBEAR: "3X Short TomoChain Token",
  DVX: "Derivex",
  KYSC: "KYSC Token",
  BYTE: "BTC Network Demand Set II",
  CHADLINK: "Chad Link Set",
  ETH20SMACO: "ETH 20 Day MA Crossover Set",
  ETHMACOAPY: "ETH 20 Day MA Crossover Yield Set",
  ETHEMAAPY: "ETH 26 EMA Crossover Yield II",
  ETHBTCRSI: "ETH/BTC RSI Ratio Trading Set",
  LINKETHPA: "ETH/LINK Price Action Candlestick Set",
  ETHPA: "ETH Price Action Candlestick Set",
  ETHRSI6040: "ETH RSI 60/40 Crossover Set",
  ETHRSIAPY: "ETH RSI 60/40 Yield Set",
  FLEXETHBTC: "FlexETH/BTC Set",
  INTRATIO: "Intelligent Ratio Set",
  LINKETHRSI: "LINK/ETH RSI Ratio Trading Set",
  LINKPT: "LINK Profit Taker Set",
  sLINK: "sLINK",
  SXAU: "sXAU",
  SXTZ: "sXTZ",
  SBNB: "sBNB",
  BTY: "Bityuan",
  XAUTBEAR: "3X Short Tether Gold Token",
  YFT: "Toshify.finance",
  HL: "HL Chain",
  AVC: "AVCCOIN",
  BTSC: "BTS Coin",
  sBTC: "sBTC",
  DTOP: "DTOP Token",
  DZAR: "Digital Rand",
  ESWA: "EasySwap",
  GTF: "GLOBALTRUSTFUND TOKEN",
  IT: "IDCM Token",
  GPO: "Galaxy Pool Coin",
  IQC: "Intelligence Quickly Chain",
  KAL: "Kaleido",
  LVH: "LoveHearts",
  ALD: "Aludra Network",
  UENC: "UniversalEnergyChain",
  RNX: "ROONEX",
  FRENS: "Frens Community",
  OWL: "OWL Token (StealthSwap)",
  EQMT: "Equus Mining Token",
  TRUMPWIN: "Trump Wins Token",
  SVB: "Sendvibe",
  CHERRY: "cherry",
  TOB: "Tokens of Babel",
  SCHA: "Schain Wallet",
  LOVE: "Love Coin",
  CHOP: "Porkchop",
  AHT: "AhaToken",
  CAMO: "Camouflage.eth",
  KSS: "Krosscoin",
  DACC2: "DACC2",
  LOOP: "LOOPREX",
  MIKS: "MIKS COIN",
  ORBI: "Orbicular",
  BICAS: "BitherCash",
  NEWTON: "Newtonium",
  OCP: "OC Protocol",
  LIB: "Libera",
  BLZN: "Blaze Network",
  WENB: "Wenburn",
  PDF: "Port of DeFi Network",
  QTBK: "Quantbook",
  STEEL: "Hands of Steel",
  CEDS: "CEDARS",
  FACT: "Fee Active Collateral Token",
  BAST: "Bast",
  SOFI: "Social Finance",
  WTF: "Walnut.finance",
  SAL: "SalmonSwap",
  YFIB: "YFIBusiness",
  YFBETA: "yfBeta",
  LIVE: "TRONbetLive",
  YFUEL: "YFUEL",
  YFP: "Yearn Finance Protocol",
  FRMS: "The Forms",
  FSP: "FlashSwap",
  gKIMCHI: "GOD KIMCHI",
  ENCX: "Enceladus Network",
  YFIKING: "YFIKING,FINANCE",
  MAFI: "Mafia.Network",
  MAKI: "Maki Finance",
  PRDX: "Predix Network",
  YFI2: "YEARN2.FINANCE",
  FHSE: "Farm House Finance",
  NYB: "New Year Bull",
  OBIC: "OBIC",
  ON: "OFIN Token",
  YMEN: "Ymen.Finance",
  XMM: "Momentum",
  YFKA: "Yield Farming Known as Ash",
  TRBT: "Tribute",
  ATIS: "Atlantis Token",
  BXIOT: "bXIOT",
  YFARMER: "YFarmLand Token",
  FAME: "Fame",
  VNS: "Va Na Su",
  MYFI: "MYFinance",
  YFMS: "YFMoonshot",
  RSP: "Resoper",
  XSP: "XSwap",
  PoSH: "Shill & Win",
  LNOT: "Livenodes Token",
  RI: "RI Token",
  FNSP: "Finswap",
  AXI: "Axioms",
  IYF: "IYF.finance",
  ISDT: "ISTARDUST",
  DCD: "Digital Currency Daily",
  JUICE: "Moon Juice",
  UTY: "UnityDAO",
  DFF: "DeFi Firefly",
  SWSH: "SwapShip",
  SPKL: "SpokLottery",
  BUCC: "Buccaneer V2",
  CXN: "CXN Network",
  YLAND: "Yearn Land",
  TMC: "Trading Membership Community",
  AHF: "AmericanHorror.Finance",
  YFE: "YFE Money",
  PJM: "Pajama.Finance",
  YFMB: "YFMoonBeam",
  YF4: "Yearn4 Finance",
  RAUX: "ErcauX",
  ZAC: "ZAC Finance",
  CYTR: "Cyclops Treasure",
  BLV: "Bellevue Network",
  YFOS: "YFOS.finance",
  STPL: "Stream Protocol",
  DARK: "Dark.Build",
  YFPRO: "YFPRO Finance",
  HAUT: "Hauteclere Shards",
  JVZ: "Jiviz",
  THIRM: "Thirm Protocol",
  YFED: "YFED.Finance",
  EFAR: "Fridn",
  CRAFT: "deCraft Finance",
  xMOON: "r/CryptoCurrency Moons",
  TAVITT: "Tavittcoin",
  LOAD: "Load Network",
  WBIND: "Wrapped BIND",
  PSHP: "Payship",
  SSL: "Sergey Save Link",
  YFDOT: "Yearn Finance DOT",
  BTCDOM: "Bitfinex Bitcoin Dominance Perps",
  YRISE: "yRise Finance",
  DXIOT: "dXIOT",
  PAYOU: "Payou Finance",
  MCN: "McNetworkDefi",
  XAT: "ShareAt",
  yBAN: "BananoDOS",
  BTNYX: "BitOnyx",
  NAZ: "Naz Coin",
  YFICG: "YFI CREDITS GROUP",
  BSWAP: "BSCswap",
  LMS: "Lumos",
  PXUSD_MAR2021: "pxUSD Synthetic USD Expiring 1 April 2021",
  MEGA: "MegaCryptoPolis",
  HD: "HubDao",
  BLOODY: "Bloody Token",
  YYFI: "YYFI.Protocol",
  GBCR: "Gold BCR",
  xDOT: "DotBased",
  YEET: "$YEET",
  stFIRO: "stakedFIRO",
  SMPL: "Smpl foundation",
  SERGS: "SERGS",
  ZLOT: "zLOT Finance",
  FLA: "Fiola",
  "7UP": "7up Finance",
  PICA: "PicaArtMoney",
  WHOLE: "wormhole.finance",
  XFII: "XFII",
  SVN: "7Finance",
  BST1: "Blueshare Token",
  VOLTS: "Volts.Finance",
  KNV: "Kanva",
  PBS: "PBS Chain",
  FCR: "Fromm Car",
  DEFO: "DefHold",
  XIF: "X Infinity",
  BMP: "Brother Music Platform",
  LCT: "Light Coin Exchange Token",
  TOM: "TOM Finance",
  UNOS: "UnoSwap",
  LK3R: "Lock3r Network",
  WZEC: "Wrapped ZEC",
  OVOA: "TheOvOrideA",
  PROPHET: "Prophet",
  LELE: "LeLeFoodChain",
  ENOL: "Ethanol",
  ALLWIN: "AllWin DeFi",
  NAR: "Narwhalswap",
  PSK: "PoolStake",
  PUNT: "Greenheart Punt",
  YFID: "YFIDapp",
  WAV3: "WAV3",
  GVY: "Groovy Finance",
  ELYX: "Elynet",
  YETI: "Yearn Ecosystem Token Index",
  GRAIN: "Grain",
  ARVO: "ARVO",
  Z502: "502 Bad Gateway Token",
  vXRP: "Venus XRP",
  ACX: "AC Index",
  VLO: "VELO Token",
  YPLT: "yplutus",
  HNY: "Honey",
  BNFI: "Blaze DeFi",
  NFUP: "Natural Farm Union Protocol",
  $PIR: "PIRANHAS",
  WSF: "WallStreet.Finance",
  R34P: "R34P",
  GRPL: "Golden Ratio Per Liquidity",
  TAXI: "Taxi",
  OFT: "Orient",
  "3XT": "3XT TOKEN",
  UNL: "unilock.network",
  CBANK: "CryptoBank",
  DFR: "Diffract Finance",
  HYPE: "Supreme Finance",
  MIC: "MITH Cash",
  LBD: "LinkBased",
  ZMT: "Zipmex",
  BCC: "Basiscoin Cash",
  BCS: "Basiscoin Share",
  SPAD: "Space Dollar",
  MCP: "My Crypto Play",
  ZAI: "Zero Collateral Dai",
  DCL: "Delphi Chain Link",
  vFIL: "Venus Filecoin",
  VAULTZ: "Vaultz",
  TRONX: "Tronx Coin",
  FCX: "Fission Cash",
  DAIQ: "Daiquilibrium",
  DSTR: "Dynamic Supply Tracker",
  DST: "Dynamic Supply",
  PEGS: "PegsShares",
  YSKF: "Yearn Shark Finance",
  STXEM: "stakedXEM",
  ZZZV2: "zzz.finance v2",
  QFI: "QFinance",
  XUSD: "XUSD Stable",
  NAWA: "Narwhale.finance",
  PSY: "Psychic",
  USDEX: "USDEX",
  U8D: "Universal Dollar",
  GLAD: "Gladius Finance",
  VBETH: "Venus BETH",
  ELE: "Elevate",
  WSHIFT: "Wrapped Shift",
  $TRDL: "Strudel Finance",
  "MD+": "MoonDayPlus",
  sYFL: "YFLink Synthetic",
  OT: "Option Token",
  DEFLA: "Defla",
  DEFI5: "DEFI Top 5 Tokens Index",
  DFST: "DeFiStarter",
  BDAY: "Birthday Cake",
  MYTHIC: "Mythic Finance",
  SIMBA: "SIMBA Storage",
  MBTC: "mStable BTC",
  PRQBOOST: "Parsiq Boost",
  "PUNK-BASIC": "Punk Basic",
  BASX: "Basix",
  MCBASE: "McBase Finance",
  DOGY: "DogeYield",
  QTF: "Quantfury Token",
  CORD: "CORD.Finance",
  WTR: "Water Finance",
  wSZO: "ShuttleOne",
  CAD: "Candy Protocol",
  aENJ: "Aave Enjin",
  WEATHER: "Weather Finance",
  ALH: "AlloHash",
  CADC: "CAD Coin",
  SLME: "Slime Finance",
  MINTY: "Minty Art",
  BNKY: "Bonkey dAPP",
  UTL: "Utile Network",
  ADAO: "Revelation Finance",
  NAUT: "Astronaut",
  SOJU: "Soju Finance",
  HEROES: "Crypto Heroes",
  CELEB: "CELEBPLUS",
  DOG: "UnderDog",
  BASI: "ASI.finance",
  NFTP: "NFT POOL",
  NTRC: "Neoteric",
  MVT: "Movement DAO",
  ASI: "ASI finance",
  ASSY: "ASSY Index",
  CRETH2: "Cream ETH 2",
  RPEPE: "Rare Pepe",
  KLON: "Klondike Finance",
  MAUSDC: "Matic Aave Interest Bearing USDC",
  MAUSDT: "Matic Aave Interest Bearing USDT",
  MAAAVE: "Matic Aave Interest Bearing AAVE",
  MAYFI: "Matic Aave Interest Bearing YFI",
  MALINK: "Matic Aave Interest Bearing LINK",
  STKR: "StakerDAO",
  PEPR: "Pepper Finance",
  MARKO: "Marko Finance",
  wxDai: "Wrapped XDAI",
  SATS: "Satoshi",
  DETO: "Delta Exchange Token",
  MERO: "Mero",
  ALIA: "XANALIA",
  FORCE: "Force DAO",
  OCTI: "Oction",
  SEFI: "SEFI",
  EURXB: "EURxb",
  SQRT: "Sqrtle Finance",
  ORK: "Orakuru",
  WOLF: "moonwolf.io",
  MGB: "Magic Balancer",
  PFL: "Professional Fighters League Fan Token",
  TORJ: "Torj.world",
  KYTE: "Kambria Yield Tuning Engine",
  UBA: "Unbox.Art",
  BSJ: "BasenjiDAO",
  STRIKE: "StrikeCoin",
  ZIOT: "ziot Coin",
  HUGO: "Hugo Finance",
  KILI: "KILIMANJARO",
  RFF: "RedFox Finance",
  EC2: "EmploymentCoin",
  ZTB: "ZTB",
  SAT: "SatisFinance Token",
  BABY: "Babytoken",
  vDOGE: "Venus Dogecoin",
  vADA: "Venus Cardano",
  AGVE: "Agave",
  KRILL: "Krill",
  WAVAX: "Wrapped AVAX",
  GOZ: "G\u00f6ztepe S.K. Fan Token",
  HAWK: "HawkDex",
  "2GOSHI": "2GoShi",
  PolyMoon: "PolyMoon",
  CRFI: "CrossFi",
  VSPACEX: "Ito Vault",
  AABC: "AABC Token",
  RASTA: "Rasta Finance",
  ARI: "Arise Finance",
  WSTA: "Wrapped Statera",
  TCAP: "Total Crypto Market Cap Token",
  ELONGD: "Elongate Deluxe",
  SNOB: "Snowball",
  XAVA: "Avalaunch",
  DOGEDAO: "DogeDao Finance",
  YUMMY: "YUMMY",
  GTON: "Graviton",
};
